...
Fetch tags, check out the selected stable tag, install the frozen lockfile, and apply versioned database migrations. Set the same release channel and revision for the Dashboard, API, Agent Gateway, and Worker, then restart them from that checkout.
