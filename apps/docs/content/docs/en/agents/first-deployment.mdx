...
Build & Deploy prepares a separate Release, installs the project's locked dependency graph, injects the platform observer and sandbox integration, starts an isolated Deployment, and waits for health. It never stops or reuses the current stable target as part of a successful deploy.
