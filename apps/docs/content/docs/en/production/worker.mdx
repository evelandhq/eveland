...
Configure the environment file before starting the service. At minimum, align database access, `/var/lib/eveland`, application encryption, Agent domains, Agent Gateway service authentication, workflow Postgres, scheduler secrets, release identity, and `EVELAND_RUNTIME=systemd` with the core services.
