...
Session collection is independent of Playground. Injected Eve hooks use Eveland-private OpenTelemetry providers and send standard OTLP to the managed Collector; Built-in projects the Agent LogRecords into Postgres. Direct Agent requests, Playground, schedules, and child sessions can all appear in the same history. User instrumentation and exporters remain unchanged.
...
Telemetry failure must not make an Agent turn fail. The Collector gives Built-in and every external exporter an independent persistent retry queue. Built-in receive health is visible in System observability settings.
