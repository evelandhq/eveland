...
The supported production topology separates the core services from privileged runtime control. The Dashboard, API, Agent Gateway, and Postgres form the core services; a host Worker starts Eve deployments as isolated systemd services. Agent ports stay on private loopback, while the Agent Gateway owns the stable and preview hosts.
