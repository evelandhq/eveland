...
含 Secret 的 authentication field 选择 Project Secret。API 在每次 Create、Continue、Cancel 与 Stream 请求时解析当前 Reference；Credential 不进入 Web 或 Gateway 存储。Shared Agent Environment 不能作为 Playground authentication credential。
