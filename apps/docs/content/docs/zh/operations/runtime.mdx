...
两种 Runtime Adapter 都会把 `EVELAND_MEMORY_MAX`、`EVELAND_CPU_QUOTA` 与 `EVELAND_TASKS_MAX` 应用到每个 Deployment cgroup：Docker 对应 `--memory`、`--cpus` 与 `--pids-limit`，systemd 对应 `MemoryMax`、`CPUQuota` 与 `TasksMax`。注入的 bwrap `run()` 命令还会在 `EVELAND_SANDBOX_RUN_TIMEOUT_MS` 后停止（默认 10 分钟）；需要长期存活的 authored process 必须使用 `spawn()`。设置时需要为 Build、Postgres、核心服务与并发 Cold Start 留出余量。
