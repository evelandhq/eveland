...
Session 采集独立于 Playground。注入的 Eve Hook 将 Durable Envelope 写入文件系统 Outbox，Collector 再将其投影到 Postgres。直接 Agent 请求、Playground、Schedule 与 Child Session 都可以进入同一历史。
...
- Provider 或 Gateway 在有报告时提供的 Cost

缺失 Usage 会明确保持缺失，Eveland 不做估算。At-least-once Envelope Delivery 不会重复累计已投影 Usage。

## Observer 健康

Observer 故障不能让 Agent Turn 失败。Envelope 会保留等待恢复，Collector Health 则与控制面 Health 分开报告 Degraded 状态。
