...
export async function importGitSource(input: ImportGitInput): Promise<void> {
  ...
  const configuredAttempts =
    input.maxAttempts ?? Number(process.env.EVELAND_GIT_CLONE_MAX_ATTEMPTS ?? 3);
  const maxAttempts = Number.isFinite(configuredAttempts)
    ? Math.max(1, Math.floor(configuredAttempts))
    : 1;
  const configuredDelay =
    input.retryDelayMs ?? Number(process.env.EVELAND_GIT_CLONE_RETRY_DELAY_MS ?? 1_000);
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      throw new Error(
        detail
          ? `Repository fetch failed: ${detail}`
          : "Repository fetch failed. Check the repository URL, credentials, and worker network, then retry.",
        {
          cause: error,
        },
      );
...
function isTransientGitError(error: unknown, detail: string): boolean {
  return (
    ...
    /could not resolve host|failed to connect|connection (?:reset|timed out)|tls|http (?:500|502|503|504)/i.test(
      detail,
    )
  );
}
