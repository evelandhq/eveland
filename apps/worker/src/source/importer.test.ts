...
describe("git source importer", () => {
  ...
  test("fails with an actionable error when git clone exceeds its timeout", async () => {
    ...
    await expect(
      ...
      "Repository fetch timed out after 50ms. Check the worker network, proxy, DNS, or repository availability, then retry.",
    );
  });
  ...
  test("removes a partial source directory when git clone fails", async () => {
    ...
    await expect(
      importGitSource({ gitUrl: "https://example.com/agent.git", targetDir, maxAttempts: 1 }),
    ).rejects.toThrow();
...
mkdir -p "$5"`,
  ...
  test("reports git stderr without exposing URL credentials", async () => {
    ...
    await useFakeGit(
      root,
      "echo 'fatal: unable to access https://secret-token@example.com/agent.git' >&2\nexit 1",
    );
    ...
    expect(failure).toBeInstanceOf(Error);
    expect((failure as Error).message).toContain(
      "fatal: unable to access https://***@example.com/agent.git",
    );
  ...
  test("retries a transient network failure and succeeds", async () => {
    ...
    await useFakeGit(
      root,
      `[ -f "${attemptsPath}" ] && count=$(cat "${attemptsPath}") || count=0
count=$((count + 1))
