...
describe("ensureDeploymentActive", () => {
  ...
  test("retires a stale RuntimeInstance without un-archiving its Deployment", async () => {
    const store = createTestStore();
    const project = await store.createProject({ name: "Archived Agent", importKind: "zip" });
    const importJob = await store.claimNextJob("fixture-import");
    await store.completeJob(importJob!.id);
    const revision = await store.recordSourceRevision({
      projectId: project.id,
      kind: "zip",
      sourcePath: "/tmp/archived-agent",
      summary: {},
      envVars: [],
      files: [],
      schedules: [],
    ...
    const deployment = await store.recordDeployment({
      projectId: project.id,
      sourceRevisionId: revision.id,
      imageTag: "fixture:archived",
      containerName: "fixture-archived",
      internalPort: 3000,
      hostPort: 41996,
      runtimeKind: "docker",
    });
    ...
    // archive_deployment stops the process and retires the Deployment, but leaves
    // its RuntimeInstance rows for this sweeper to clean up.
