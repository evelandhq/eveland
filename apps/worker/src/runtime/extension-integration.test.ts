...
test("keeps the Extension integrator compatible with the oldest supported Eve 0.49 manifest", async () => {
  const releaseDir = await mkdtemp(path.join(os.tmpdir(), "eveland-extension-oldest-"));
  roots.push(releaseDir);
  const extensionPackageRoot = path.join(releaseDir, "packages/crm");
  await writeFixtureExtension(extensionPackageRoot, oldestEvePackageRoot, "0.49.0", false);
  await execFileAsync(process.execPath, [oldestEveBin, "extension", "build"], {
    cwd: extensionPackageRoot,
  });
  await writeConsumer(releaseDir, extensionPackageRoot, oldestEvePackageRoot, "0.49.0");
