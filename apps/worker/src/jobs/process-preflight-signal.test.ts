...
const { importGitSource, getGitCommitSha, scanEveSource } = vi.hoisted(() => ({
  importGitSource: vi.fn<(input: { signal?: AbortSignal }) => Promise<void>>(
    async () => undefined,
  ),
  getGitCommitSha: vi.fn<
    (sourcePath: string, signal?: AbortSignal) => Promise<string>
  >(async () => "abc123"),
...
describe("source preflight fencing", () => {
  test("threads the heartbeat abort signal through the git import", async () => {
    ...
    await expect(
      processNextSourcePreflight(store, "signal-worker"),
    ).resolves.toBe(true);
    ...
    expect(importGitSource.mock.calls[0]![0].signal).toBeInstanceOf(
      AbortSignal,
    );
