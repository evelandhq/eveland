...
/**
 * Writes the outcome of a restart onto the Deployment row it owns.
 *
 * The runtime reconciler and the idle reaper both write this column while a
 * restart sits between its stop and its health check, so the restart has to
 * overwrite their verdict -- otherwise a healthy process keeps a "failed" or
 * "stopped" row that blocks promotion and gets the unit reaped as an orphan.
 * `draining` and `archived` are the exception: those are control-plane
 * decisions about whether this Deployment may serve at all, and a restart
 * (which secret and identity changes fan out automatically) must not quietly
 * cancel one.
 */
async function settleRestartedDeploymentStatus(
  ...
  if (!current || current.status === "draining" || current.status === "archived")
    return;
  await store.updateDeploymentStatus(deploymentId, status);
}
