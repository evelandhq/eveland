    ...
    expect(config.processors["filter/builtin_capacity"]).toEqual({
      ...
      metric_conditions: [
        ...
        [
          ...
          'metric.name != "eveland.host.cpu.logical.count"',
          'metric.name != "eveland.postgres.connections.usage"',
          'metric.name != "eveland.postgres.connections.limit"',
          'metric.name != "eveland.postgres.agent_pool_size"',
        ].join(" and "),
