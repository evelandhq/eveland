...
} from "./process-source-preflight.js";
...
describe("processNextJob", () => {
  ...
  test("fails a cold activation closed when the workflow topology is unclassified", async () => {
    const store = createTestStore();
    const sourcePath = await createFixtureEveProject("0.58.1");
  ...
  test("does not fail the Project when an old Deployment activation fails", async () => {
    ...
    await rm(missingSourcePath, { recursive: true, force: true });
    const currentSourcePath = await createFixtureEveProject("0.58.1");
