...
describe("schedule scale-to-zero integration harness", () => {
  test("keeps the real fixture on the Eve 0.24.x line and runs it in the Linux smoke", async () => {
    ...
    expect(JSON.parse(fixturePackage)).toMatchObject({ dependencies: { eve: "0.24.x" } });
