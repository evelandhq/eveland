...
describe("systemd integration smoke", () => {
  test("uses Eve projects for both the healthy and timeout-cleanup fixtures", async () => {
    ...
    expect(smokeScript.match(/dependencies: \{ eve: OLDEST_VERIFIED_EVE_VERSION \}/g)).toHaveLength(
      1,
    );
    expect(smokeScript.match(/dependencies: \{ eve: LATEST_VERIFIED_EVE_VERSION \}/g)).toHaveLength(
      1,
    );
