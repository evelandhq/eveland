...
// durable session workspace. The fixture authors its own Eve 0.64 sandbox on
// a Docker image the host never pulls, so every pass also proves Eveland
// redirected it. Two layers combine to prove this, and both are required for
// the script to report success:
//   - A deterministic check that imports the injected platform module and the
//     vendored bwrap provider directly, deliberately bypassing eve's own
//     session/runtime APIs. This proves the platform module builds a bwrap
//     environment, that `eve build` prepared the template on bwrap with the
//     authored preparation, and that EVELAND_SANDBOX_CACHE_DIR keeps the
//     session workspace, but says nothing about whether eve's compiled runtime
//     ever actually loads the redirected module.
...
/**
 * The deterministic core of this test: proves the injected platform module
 * builds bwrap environments, that eve prepared the template on bwrap, and that
 * the vendored provider honors EVELAND_SANDBOX_CACHE_DIR on this host,
 * without going through eve's session/runtime APIs at all. Written into the release, then run as root writes it, but
 * executed as the deployment user under the same hardening the deployed
 * unit gets.
 */
function buildCheckScript(): string {
  return `// Generated by agent-sandbox-e2e.ts. Verifies the injected platform module
// (.eveland/sandbox-platform/platform.js) builds bwrap environments, that eve
// prepared the root template on bwrap, and that the vendored provider keeps a
// durable session in EVELAND_SANDBOX_CACHE_DIR -- deliberately bypassing
// eve's own session/runtime APIs so this check depends only on
// @evelandhq/sandbox-bwrap and the injected modules, never on eve's model loop.
import assert from "node:assert/strict";
...
const importRelease = (relative) => import(pathToFileURL(path.join(releaseDir, relative)).href);

const platform = await importRelease(".eveland/sandbox-platform/platform.js");
assert.equal(platform.platformEnvironment().provider, "bwrap", "the platform module must build bwrap environments");

const manifest = JSON.parse(
  await readFile(path.join(releaseDir, ".output/.eve/compile/sandbox-prepared-artifacts.json"), "utf8"),
);
const root = manifest.entries.find((entry) => entry.nodeId === "__root__");
assert.equal(root?.providerName, "bwrap", \`eve must prepare the root sandbox on bwrap, got \${JSON.stringify(root)}\`);

const { createBwrapSandboxProviderDefinition } = await importRelease(".eveland/sandbox-bwrap/index.js");
const cacheDir = process.env.EVELAND_SANDBOX_CACHE_DIR;
assert.ok(cacheDir, "EVELAND_SANDBOX_CACHE_DIR must be set");
const environment = createBwrapSandboxProviderDefinition().environment({ cacheDir });
const ctx = {
  host: {
  ...
  session: {
    ...
    id: ${JSON.stringify(SESSION_KEY)},
    turn: { id: "e2e-check", sequence: 0 },
  },
  storagePath: path.join(releaseDir, ".eve", "sandbox-cache"),
};
const statePath = path.join(cacheDir, "e2e-provider-state.json");
const markerContent = ${JSON.stringify(MARKER_CONTENT)};

const mode = process.env.E2E_CHECK_MODE;
let handle;
if (mode === "seed") {
  const started = await environment.start(ctx, undefined, root.artifact);
  handle = started.handle;
  await writeFile(statePath, JSON.stringify(started.state));
} else if (mode === "verify") {
  // Resumed against the second Release's artifact: the workspace belongs to
  // the session, not to the Release that first started it.
  handle = await environment.resume(ctx, root.artifact, JSON.parse(await readFile(statePath, "utf8")));
} else {
  throw new Error('E2E_CHECK_MODE must be "seed" or "verify"');
}
try {
  if (mode === "seed") {
    const prepared = await handle.sandbox.readTextFile({ path: "eveland-prepare.txt" });
    assert.equal(
      prepared,
      ${JSON.stringify(`authored-prepare-saw:${INITIAL_SEED_CONTENT}`)},
      "the authored prepare must have run inside the template during eve build",
    );
    const result = await handle.sandbox.run({ command: "echo hello-from-sandbox" });
    ...
    await handle.sandbox.writeTextFile({ path: "e2e-marker.txt", content: markerContent });
    const restrict = await handle.sandbox.run({
    ...
    const content = await handle.sandbox.readTextFile({ path: "e2e-marker.txt" });
    assert.equal(content, markerContent, "durable session marker did not survive the redeploy");
    const result = await handle.sandbox.run({ command: "echo still-alive" });
    assert.equal(result.exitCode, 0, \`echo failed: \${result.stderr}\`);
...
async function runHttpTurnOnce(input: {
  ...
  const expectedPrepareContent = `authored-prepare-saw:${input.expectedSeedContent}`;
  const message =
    ...
    `test "$(cat eveland-prepare.txt)" = ${JSON.stringify(expectedPrepareContent)} && ` +
    `test "$(cat eveland-selector.txt)" = authored-selector-ran && ` +
...
try {
  ...
  assert.ok(
    buildLogs.includes(
      "Redirected the project's authored sandbox (agent/sandbox/sandbox.ts) to Eveland's bwrap provider",
    ),
    `build log missing the redirected-authored-sandbox line:\n${buildLogs}`,
  );
  assert.ok(
    !buildLogs.includes("no agent/ directory"),
    `build log must not claim the project has no agent/ directory:\n${buildLogs}`,
  );

  const releaseDir1 = releaseDirFor(project.id, deployment.releaseId);
  const authoredSandbox = await readFile(
    path.join(releaseDir1, "agent", "sandbox", "sandbox.ts"),
    "utf8",
  );
  assert.ok(
    authoredSandbox.includes('from "../../.eveland/sandbox-platform/docker.js"'),
    `${releaseDir1}/agent/sandbox/sandbox.ts must import the platform shim:\n${authoredSandbox}`,
  );
  assert.ok(
    !(await pathExists(path.join(releaseDir1, "agent", "sandbox", "sandbox.js"))),
    `${releaseDir1}/agent/sandbox/sandbox.js must not shadow the authored module`,
  );
  assert.ok(
    await pathExists(path.join(releaseDir1, ".eveland", "sandbox-platform", "platform.js")),
    `${releaseDir1}/.eveland/sandbox-platform/platform.js must exist`,
  );
  assert.ok(
  ...
  // --- B.4: deterministic proof the platform module selects OUR provider,
  // eve prepared the template on it, and the session workspace lives in the
  // cache dir, independent of eve's session API. --------------------------
  ...
  // deployed runtime (not just the injected modules in isolation) runs the
  // redirected sandbox on bwrap for a real turn. Throws on failure. -------
  ...
  console.log(
    `Eve's deployed runtime ran the redirected sandbox on bwrap for a live turn -- ${liveTurnDetail}`,
  );
  ...
  assert.ok(
    await pathExists(path.join(releaseDir2, "agent", "sandbox", "sandbox.ts")),
    `${releaseDir2}/agent/sandbox/sandbox.ts must exist`,
  );
  assert.equal(
    await readFile(
