...
export const ToolContent = ({ className, ...props }: ToolContentProps) => (
  <CollapsibleContent
    className={cn(
      "data-[starting-style]:slide-in-from-top-2 data-[ending-style]:fade-out-0 data-[ending-style]:slide-out-to-top-2 space-y-4 py-2 text-popover-foreground outline-none data-[ending-style]:animate-out data-[starting-style]:animate-in",
      className,
    )}
    {...props}
  />
...
export const BashToolContent = ({
  ...
  const stdoutValue = getRecordValue(output, "stdout");
  const stdout =
    typeof stdoutValue === "string" ? stdoutValue : typeof output === "string" ? output : "";
  const stderrValue = getRecordValue(output, "stderr");
  const stderr = typeof stderrValue === "string" ? stderrValue : (errorText ?? "");
