...
export function DeploymentTrafficActions({
  ...
  const canSplit = stableRouteId && productionDeploymentId && productionDeploymentId !== deploymentId && status === "running";
  return (
    <div className="flex flex-wrap items-center justify-end gap-2">
      <Button size="sm" variant="outline" disabled={pending !== null || status !== "running"} onClick={() => run("promote", () => promoteDeployment(projectId, deploymentId))}>
        Promote / rollback
      </Button>
      {canSplit ? <>
        <Button size="sm" variant="ghost" disabled={pending !== null} onClick={() => run("90/10", () => updateRouteTargets(projectId, stableRouteId, [
          { deploymentId: productionDeploymentId, weight: 9000, variantName: "control" },
          { deploymentId, weight: 1000, variantName: "candidate" },
        ]))}>90/10</Button>
        <Button size="sm" variant="ghost" disabled={pending !== null} onClick={() => run("50/50", () => updateRouteTargets(projectId, stableRouteId, [
