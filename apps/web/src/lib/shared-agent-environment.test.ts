...
type EnvironmentModule = {
  ...
  updateSharedAgentEnvironmentEntry(
    ...
    patch: Partial<{ key: string; kind: "variable" | "secret"; value: string; configured: boolean }>,
...
async function loadEnvironmentModule(): Promise<EnvironmentModule | null> {
  return import("./shared-agent-environment").catch(() => null) as Promise<EnvironmentModule | null>;
}
...
describe("shared Agent environment form", () => {
  test("preserves configured entries without copying their values", async () => {
    ...
    expect(module?.validateSharedAgentEnvironmentDraft({
      entries: [
  ...
  test("requires a new value after changing a configured entry identity", async () => {
    const module = await loadEnvironmentModule();
    const configured = { key: "OPENAI_API_KEY", kind: "secret" as const, value: "", configured: true };

    expect(module?.updateSharedAgentEnvironmentEntry(configured, { key: "ANTHROPIC_API_KEY" }))
      .toMatchObject({ key: "ANTHROPIC_API_KEY", configured: false });
    expect(module?.updateSharedAgentEnvironmentEntry(configured, { kind: "variable" }))
      .toMatchObject({ kind: "variable", configured: false });
    expect(module?.updateSharedAgentEnvironmentEntry(configured, { value: "rotated" }))
