...
describe("browser auth API", () => {
  ...
  test("signs in with credentialed browser requests", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(1, "http://localhost:17301/api/auth/sign-in/email", {
      method: "POST",
      credentials: "include",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ email: "admin@example.com", password: "admin-password" }),
    });
    expect(fetchMock).toHaveBeenNthCalledWith(2, "http://localhost:17301/auth/session", {
      method: "GET",
      credentials: "include",
    });
  });
  ...
  test("signs out through Better Auth with a JSON request", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("http://localhost:17301/api/auth/sign-out", {
      method: "POST",
      credentials: "include",
      headers: { "content-type": "application/json" },
      body: "{}",
  ...
  test("updates the current profile through the authenticated Eveland API", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("http://localhost:17301/profile", {
      method: "PATCH",
      credentials: "include",
      headers: { "content-type": "application/json" },
  ...
  test("changes the password through the authenticated Eveland API", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("http://localhost:17301/profile/password", {
      method: "POST",
      credentials: "include",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
  ...
  test("accepts an invitation with the chosen member password", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:17301/invitations/accept",
