...
import {
  getApiBuildInfo,
  getCurrentMember,
  getSystemConfigurationDiagnostics,
} from "@/lib/server-api";
...
export default async function AboutSettingsPage() {
  ...
  const systemConfiguration =
    currentMember.role === "admin"
      ? await getSystemConfigurationDiagnostics().catch(() => null)
      : null;
  const configurationComponents =
    currentMember.role === "admin"
      ? [
          createConfigurationSnapshot("web", process.env),
          ...(systemConfiguration?.components ?? []),
        ]
      : [];
  const configurationIssues = configurationComponents
    .flatMap((component) => component.entries)
    .filter((entry) => entry.status !== "ok");
  const missingConfiguration = configurationIssues.filter(
    ...
    <div className="flex max-w-5xl flex-col gap-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Compare the build identity reported by each visible part of this instance.
          </CardDescription>
        </CardHeader>
            ...
            <TableBody>
              {[webBuild, ...(apiBuild ? [apiBuild] : [])].map((build) => (
                <TableRow key={build.component}>
                  ...
                  <TableCell>
                    <code className="font-mono">{build.revision}</code>
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline">{build.channel}</Badge>
                  </TableCell>
                </TableRow>
              ))}
              {!apiBuild ? (
      ...
      {currentMember.role !== "admin" ? (
        <Alert>
          ...
          <AlertDescription>
            Runtime configuration is available only to Team administrators.
          </AlertDescription>
        </Alert>
      ) : (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Effective values, their source, and the purpose of each supported environment
              variable.
            </CardDescription>
            <CardAction>
          ...
          <CardContent className="flex flex-col gap-4">
            <Alert>
              ...
              <AlertDescription>
                Secrets are never returned. Credentials and query values are removed from connection
                URLs.
              </AlertDescription>
            </Alert>
            ...
            {configurationIssues.length > 0 ? (
              <Alert variant={missingConfiguration > 0 ? "destructive" : "default"}>
                ...
                <AlertDescription>
                  {missingConfiguration} missing and {warningConfiguration} warning configuration
                  values were found.
                </AlertDescription>
              </Alert>
            ) : null}
