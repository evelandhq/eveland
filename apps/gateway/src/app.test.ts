...
describe("Gateway", () => {
  ...
  test("rejects public Eve session traffic for the selected unsupported deployment before proxy or activation", async () => {
    ...
    repo.getDeploymentEveVersion = vi.fn(async () => ({
      version: "0.22.6",
      expected: "0.24.x, 0.25.x, or 0.26.x" as const,
      supportedRanges: ["0.24.x", "0.25.x", "0.26.x"] as const,
      supported: false,
    ...
    for (const request of [
      ...
      await expect(response.json()).resolves.toEqual({
        ...
        detail: 'Unsupported Eve dependency "0.22.6". Eveland requires Eve 0.24.x, 0.25.x, or 0.26.x. Upgrade the project\'s "eve" dependency before importing or deploying.',
        eveVersion: {
          version: "0.22.6",
