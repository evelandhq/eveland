...
describe("Gateway", () => {
  ...
  test("rejects public Eve session traffic for the selected unsupported deployment before proxy or activation", async () => {
    ...
    repo.getDeploymentEveVersion = vi.fn(async () => ({
      version: "0.22.6",
      expected: "0.62.x or 0.65.x" as const,
      supportedRanges: ["0.62.x", "0.65.x"] as const,
      supported: false,
  ...
  test("fails closed on custom routes to a running out-of-window Deployment", async () => {
    ...
    for (const request of [
      ...
      await expect(response.json()).resolves.toMatchObject({
        error: "Unsupported Eve version",
        detail:
          'Unsupported Eve dependency "0.37.1". Eveland requires Eve 0.62.x or 0.65.x. Upgrade the project\'s "eve" dependency before importing or deploying.',
      });
    }
