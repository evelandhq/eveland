...
describe("Gateway", () => {
  test("keeps the privileged Playground path service-authenticated and uses loopback Host", async () => {
    ...
    const upstream = await startUpstream((request, response) => {
      ...
      if (request.method === "POST") {
        response.writeHead(202, {
          ...
          "x-eve-session-id": "eve_playground",
        });
        response.end(
          JSON.stringify({
            sessionId: "eve_playground",
      ...
      response.write(
        `${JSON.stringify({ type: "message.completed", data: { message: "Playground answer" } })}\n`,
      );
      response.end(
        `${JSON.stringify({ type: "turn.completed", data: { turnId: "turn_0" } })}\n`,
      );
    });
    ...
    const unauthorized = await app.request(
      "http://gateway/internal/projects/proj_1/playground",
      { method: "POST" },
    );
  ...
  test("activates a stopped Deployment for the legacy privileged Playground path", async () => {
    const upstream = await startUpstream((request, response) => {
      if (request.method === "POST") {
        response.writeHead(202, {
          "content-type": "application/json",
          "x-eve-session-id": "eve_legacy_wake",
        });
        response.end(
          JSON.stringify({
            sessionId: "eve_legacy_wake",
            continuationToken: "continue_legacy_wake",
          }),
        );
        return;
      }
      response.writeHead(200, { "content-type": "application/x-ndjson" });
      response.write(
        `${JSON.stringify({ type: "message.completed", data: { message: "Awake" } })}\n`,
      );
      response.end(
        `${JSON.stringify({ type: "turn.completed", data: { turnId: "turn_0" } })}\n`,
      );
    ...
    const activationClient = {
      activate: vi.fn(async () => ({
        leaseId: "lease_legacy_playground",
