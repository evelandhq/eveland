...
(Backfilled slugs always carry a deterministic global row-number suffix, padded to at least 12 digits and sized before prefix truncation, so the index build cannot collide even when names share long prefixes. Fresh databases get the column via `db:push` before any rows exist.)
