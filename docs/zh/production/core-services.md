...
**发布到宿主机的 `5432` 绝不能从宿主机之外访问。** 它的存在是为了让宿主机服务（Worker、Workflow Dispatcher）和已部署的 Agent 容器（通过 `host.docker.internal`）访问数据库，而它携带的是众所周知的默认凭据。必须在宿主机防火墙上阻断所有非本地网络对它的访问——参见[网络](/zh/docs/production/networking)。
