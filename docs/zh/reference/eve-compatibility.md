...
在 Eve 发布稳定 Compatibility Contract 之前，Eveland 只支持通过完整兼容矩阵的 Minor Line，并显式变更该窗口。代码中的产品契约支持 `0.54.x` 与 `0.55.x`，验证版本为 `0.54.5` 与 `0.55.0`。Eve 0.52 与 0.53（2026-09-15 随 0.55 进入窗口、窗口由三条线收窄为两条而同时退出）、Eve 0.50 与 0.51（2026-09-12 同时退出）、Eve 0.49（2026-09-07 退出）、Eve 0.48（2026-09-02 窗口滑到它之后即被 0.49.0 取代、被跳过，没有任何 Deployment 在其上运行过）以及 Eve 0.47 及更早版本不再允许 Import、Build、Restart、Activation、Playground、Agent Gateway 或 Schedule Execution。

项目 `package.json` 中允许的 Eve 依赖声明形式为：受支持线内的精确 Patch、锚定在受支持 Minor Patch 上的 `~`/`^` Range，以及 `0.54` / `0.54.x` / `0.54.*`、`0.55` / `0.55.x` / `0.55.*`。缺少 Eve 依赖、跨 Minor 的宽泛 Range 或任何可能解析到窗口之外的声明都会 Fail Closed。项目 Overview、Source 与 Playground 会显示当前 Deployment 对应 Source Revision 的 Eve 依赖版本与平台要求。

窗口现在是两条线：`0.55.x` 于 2026-09-15 进入，同一轮里窗口由三条线收窄为两条，`0.52.x` 与 `0.53.x` 随之同时退出，仍锁定其中任一条的项目必须在下一次 Build 或 Restart 之前迁到受支持的线。已在运行、其 Release 由这两条线构建的 Deployment，从 Eveland 带上这个窗口起就不再可服务：Agent Gateway 对它回答 409，Activation 拒绝它，平台会把仍停在它上面的 Workflow Run 直接结算，而不是继续重试。Eve 无法在另一个 Eve 版本下继续同一个 Session，因此重新构建只会开启新的 Session、无法接续旧的；请在升级 Eveland 之前把这类项目用 `0.54.x` 或 `0.55.x` 重新构建并 Promote，并预期仍绑定在旧 Deployment 上的对话会结束。`0.54.x` 留在窗口内并重新验证到 `0.54.5`。窗口内每条线都能以 Delivery Id 确认消息，因此每个被接受的 Deployment 都能从 Playground 连上；0.52.0–0.52.2 曾处于的「被接受但 Playground 连不上」区间已随 0.52 一起消失。两条 Minor 相邻，因此其 Range 是一个连续区间，而不是带缺口窗口所需的多段并集。`0.48.x` 从未通过验证（0.49.0 在数小时内即将其取代，更早的 0.46、0.43 与 0.40/0.41 亦然），继续连同 `0.53.x`、`0.52.x`、`0.51.x`、`0.50.x`、`0.49.x`、`0.47.x` 及更早版本一起落在下限之下；`0.56.x` 在通过兼容矩阵之前也不会被接纳。

UI 仅将最新支持线 `0.55.x` 标为绿色。Eve 0.54.x 保持可运行，不过会以红色显示并提醒升级；不受支持的版本同样显示为红色且继续阻断。
...
- `tool` 与 `dynamicTool` 两个 Extension 契约已丢弃 0.53.0 引入的版本之前的全部版本：针对 0.52 或更早预编译的 Extension 包会在 Build 时被拒绝、必须重建；Eveland 自行注入的 Extension 从源码编译，不受影响。Eve CLI 默认向 Vercel 上报使用情况，除非设置 `EVE_TELEMETRY_DISABLED`；Eveland 为其运行的每次 Build 与每个 Deployment 都设置了它。
...
- **Eve 0.54.x（验证版本 `0.54.5`）**：经过 0.54.4 与 0.54.5 两个 Patch 之后的 0.54 线。`ctx.model?.id` 向动态 Model、Subagent、Tool、Skill 与 Instruction 解析器暴露生效的 Model。Slack Channel 可以用 `approvalChannel` 把工具审批与 `ctx.ask()` 提问路由到共享线程或触发用户的私信，Telegram 的审批回复会恢复挂起的工具。新入口 `eve/vercel` 的 `withEve` 用于 Vercel 部署下组合 Workspace Agent，对 Eveland 没有影响。ChatGPT 订阅登录优先使用已安装 Codex CLI 的 app-server，这不改变 Eveland 上 `chatgpt()` 仅限本地的规则。子 Session 现在也使用此前只有根 Session 才有的 4000 万 Input Token 默认上限（`limits.maxInputTokensPerSession: false` 仍可关闭）。Eve 自带的 `@workflow/*` Bundle 移到 `5.0.0-beta.51/21/35/44`；共享 World 不受影响，因为 Eve 只比较 Major 与 Prerelease 标签。
- **Eve 0.55.x（推荐，验证版本 `0.55.0`）**：Channel 用 `defineChannel` 新增的 `audience(input)` 钩子分类谁能观察这段对话，`eveChannel` 也接受 `audience` 选项，默认把匿名调用者归为 `public`、`user` / `service` / `runtime` 主体归为 `private`；Eveland 的 `evelandIdentity()` 会话与注入的 Scheduler Channel 都归为 `private`，对这些会话而言 Eve 内置 Trace 内容策略的行为与 0.54 一致。匿名会话（放行 `none()` 调用者的 Agent）现在归为 `public`，0.54 下则是 `unknown`，因此 Agent 自己的 `otel()` 或 `otelIntegration()` Exporter 默认会收到这类会话的 Prompt 与回复；如需只保留 Metadata，请给 `eveChannel` 传 `audience` 或配置 `tracePolicy`。Eveland 的 Observer 按自己的采集设置工作，不受影响。`metadata()` 里的 `audience` 键已弃用：仍设置它的 Channel 会经由一次性警告的回退继续工作，`ChannelAudienceMetadata` 类型已从 `eve/channels` 移除。自定义 `tracePolicy` 现在除 `audience` 外还收到 `channel`、`mode`、`environment` 与 `principalType`，默认只对 `public` 对话或 development 环境记录内容——Eveland Deployment 始终是 `production`。所有 Extension 契约都向前推进（tool 38、dynamicTool 37、channel 21、schedule 12、subagent 15、connection 18）但没有丢弃任何版本，针对 0.54 预编译的 Extension 仍被接受。持久化的 Session Inbox Wire 升到版本 7 以携带后台任务标签；旧 Session 在读取时迁移。由 Eve 自己触发的 Schedule，若 Turn 启动了后台工作，会保持打开、不发中间输出，直到结果可用再一次性交付；Eveland 的 Scheduler 通过注入的 Channel 启动 Schedule Session，因此 Eveland 上的 Schedule 仍按 0.54 的方式交付。`defineMcpClientConnection` 新增可选的 `protocolVersionDiscovery` 开关。对当前最新线，Agent 项目应刷新 Lockfile 并重新部署，才能实际获得 `0.55.0`，即便 `^0.55.0` 这样的 Range 已经允许它。
