...
- **Eve 0.54.x（验证版本 `0.54.5`）**：经过 0.54.4 与 0.54.5 两个 Patch 之后的 0.54 线。`ctx.model?.id` 向动态 Model、Subagent、Tool、Skill 与 Instruction 解析器暴露生效的 Model。Slack Channel 可以用 `approvalChannel` 把工具审批与 `ctx.ask()` 提问路由到共享线程或触发用户的私信，Telegram 的审批回复会恢复挂起的工具。新入口 `eve/vercel` 的 `withEve` 用于 Vercel 部署下组合 Workspace Agent，对 Eveland 没有影响。ChatGPT 订阅登录优先使用已安装 Codex CLI 的 app-server，这不改变 Eveland 上 `chatgpt()` 仅限本地的规则。Eve 自带的 `@workflow/*` Bundle 移到 `5.0.0-beta.51/21/35/44`；共享 World 不受影响，因为 Eve 只比较 Major 与 Prerelease 标签。
- **Eve 0.55.x（推荐，验证版本 `0.55.0`）**：Channel 用 `defineChannel` 新增的 `audience(input)` 钩子分类谁能观察这段对话，`eveChannel` 也接受 `audience` 选项，默认把匿名调用者归为 `public`、`user` / `service` / `runtime` 主体归为 `private`；Eveland 的 `evelandIdentity()` 会话与注入的 Scheduler Channel 都归为 `private`，Eve 内置 Trace 内容策略的行为与 0.54 完全一致。`metadata()` 里的 `audience` 键已弃用：仍设置它的 Channel 会经由一次性警告的回退继续工作，`ChannelAudienceMetadata` 类型已从 `eve/channels` 移除。自定义 `tracePolicy` 现在除 `audience` 外还收到 `channel`、`mode`、`environment` 与 `principalType`，默认只对 `public` 对话或 development 环境记录内容——Eveland Deployment 始终是 `production`。所有 Extension 契约都向前推进（tool 38、dynamicTool 37、channel 21、schedule 12、subagent 15、connection 18）但没有丢弃任何版本，针对 0.54 预编译的 Extension 仍被接受。持久化的 Session Inbox Wire 升到版本 7 以携带后台任务标签；旧 Session 在读取时迁移。子 Session 现在也继承此前只有根 Session 才有的 4000 万 Input Token 默认上限（`limits.maxInputTokensPerSession: false` 仍可关闭），Schedule 触发且启动了后台工作的 Turn 会保持打开、不发中间输出，直到结果可用再一次性交付。`defineMcpClientConnection` 新增可选的 `protocolVersionDiscovery` 开关。对当前最新线，Agent 项目应刷新 Lockfile 并重新部署，才能实际获得 `0.55.0`，即便 `^0.55.0` 这样的 Range 已经允许它。
