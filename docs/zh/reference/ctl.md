...
| `backups/`         | 每次执行版本升级前自动生成的数据库快照备份；`update` 完成后只保留最新 3 份（`--keep-backups N`）。 |
...
| `eveland-ctl update`            | 自动备份数据库、拉取最新稳定版本、执行数据库迁移并滚动重启全部组件。支持通过 `--version vX.Y.Z` 指定目标版本；`--keep-backups N` 指定升级完成后保留几份升级前备份（默认 3 份，含本次新生成的）。 |
