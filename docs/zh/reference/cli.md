...
| `eveland deploy [dir]` | 本地预检打包、上传代码、等待服务端沙箱构建；Promote 随上传请求一起提交，由平台在构建任务内完成。Zip 项目默认自动发布（`--no-promote` 仅保留预览）；Git 项目默认只部署预览，传入 `--promote` 才发布。 |
...
- **按项目类型决定是否 Promote**：由 Zip 上传创建的项目默认自动 Promote：构建通过后原子地将生产路由与定时任务目标切换至新部署；Promote 随部署请求提交，由平台在同一个构建任务内完成，CLI 中途退出也不会把路由和定时任务目标留在旧部署上，控制台同时发起的部署也不会被误 promote；CLI 汇报的是自己这次构建任务产出的部署。若仅需创建独立的临时测试环境，请传入 `--no-promote`。从 Git 导入的项目同样接受上传，但**默认只部署预览**：传入 `--promote` 才会把上传的版本作为热修复推上生产。发布上传版本会让生产运行仓库中不存在的代码（「热修复漂移」，见[发布与路由](/docs/agents/releases-routing)）：CLI 在发布后打印该警告，在上传前说明它替换掉的是哪个热修复；控制台会持续显示横幅，直到再次 Promote 某个从 Git 同步的版本——漂移期间的生产同步会先要求确认。
- **来源溯源**：当部署目录是 Git 检出时，CLI 会记录上传所基于的提交（`git rev-parse HEAD`）以及工作树是否有未提交改动（`git status --porcelain`），在终端打印，并在控制台的对应部署上展示（"由 _用户_ 从 CLI 上传，基于 `abc123`，含未提交改动"）。不在检出目录内或未安装 git 时不记录任何信息，上传照常进行。
