...
| `eveland deploy [dir]` | 本地预检打包、上传代码、等待服务端沙箱构建；Promote 随上传请求一起提交，由平台在构建任务内完成。Zip 项目默认自动发布（`--no-promote` 仅保留预览）；Git 项目只部署预览。 |
