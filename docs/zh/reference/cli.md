...
| `eveland deploy [dir]` | 本地预检打包、上传代码、等待服务端沙箱构建并自动发布；Promote 随上传请求一起提交，由平台在构建任务内完成。支持 `--no-promote` 仅保留预览部署。 |
...
- **默认自动 Promote**：构建通过后默认将生产流量切换至新版本。Promote 随部署请求提交，由平台在同一个构建任务内完成，CLI 中途退出也不会把路由和定时任务目标留在旧部署上，控制台同时发起的部署也不会被误 promote；CLI 汇报的是自己这次构建任务产出的部署。若仅需创建独立的临时测试环境，请传入 `--no-promote`。
