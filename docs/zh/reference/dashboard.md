...
页面展示活动成员与待接受邀请。Admin 可以：按邮箱创建七天有效、单次使用的邀请链接；刷新邀请以轮换 token 并延长有效期；复制邀请链接或撤销邀请；将成员设为 Admin / Member；为成员生成密码重置链接；移除成员（移除后立即撤销其所有登录 Session，团队项目不删除）。最后一个 Admin 不能被移除或降级。邀请链接使用 256-bit 不透明随机标识，接受后立即失效。移除成员保留底层用户账户：全新邮箱接受邀请时创建成员档案与密码；已有账户的邮箱接受邀请时呈现明确的重新加入流程，使用现有密码登录——接受邀请永远不会重置或替换该密码。忘记该密码的成员应请 Admin 生成密码重置链接。

密码重置沿用邀请模式：Admin 生成单次使用的链接（256-bit 不透明 token，24 小时有效）并在平台外自行分发——平台不发送邮件。重置页面先验证 token 再展示任何账户信息；完成重置会设置新密码（至少 12 个字符）并撤销该账户的所有登录 Session。为同一成员签发新链接会使既有链接立即失效，服务端只存储 token 的哈希，成员被移除后其链接同时失效。Admin 可以为任何成员（包括自己）签发重置链接。Better Auth 自带的恢复端点（`forget-password`、`reset-password`）仍然 404；整个流程只经过 Eveland-owned 路由。
...
- 当生产运行的是热修复上传（Git 项目已发布的 Release 来自上传而非同步）时，页面显示漂移横幅，`Sync, deploy & promote` 在用户勾选「用仓库最新提交替换该热修复」之前保持禁用；仅预览的同步与当前 Revision 的构建从不询问
- Restart deployment
- Open Playground
- 查看日志
