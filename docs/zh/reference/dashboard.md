...
页面展示活动成员与待接受邀请。Admin 可以：按邮箱创建七天有效、单次使用的邀请链接；刷新邀请以轮换 token 并延长有效期；复制邀请链接或撤销邀请；将成员设为 Admin / Member；移除成员（移除后立即撤销其所有登录 Session，团队项目不删除）。最后一个 Admin 不能被移除或降级。邀请链接使用 256-bit 不透明随机标识，接受后立即失效。
