...
导入类型在创建时确定且不可更改，但它只说明项目是如何创建的。Git 项目同样接受 `eveland deploy` 的上传，默认以预览方式部署，传入 `--promote` 则作为热修复发布；每个 Revision 都记录自己的来源，因此 Deployments 页面能区分同步的提交与上传的快照，并在同步替换热修复前给出提醒。
