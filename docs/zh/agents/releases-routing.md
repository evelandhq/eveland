...
- **来源溯源 (Provenance)**：每个 Deployment 都会显示其 Release 的源码来源：同步的提交，或是一次上传（来自 CLI 还是控制台、由谁发起、基于哪个提交、工作树是否有未提交改动）。发布到 Git 项目的上传是一次热修复：页面会提示生产已偏离仓库，部署对话框在用同步结果替换它之前会要求确认。
