...
| **修改平台自有运行时值**     | Worker 下次启动时重启所有运行中部署 | 所有 Agent               |

平台自有运行时值指由 Worker 注入、并对项目条目保留的那些：共享 workflow 数据库、Scheduler runtime secret 与 redeem URL、Identity issuer 与 JWKS URL、memory root、Sandbox 限额。之所以重启而不是放任不管：跨越这类改动持续运行的部署会无限期保留启动时的旧值，随后只在被改动值所属的子系统里失败，而 HTTP 与健康检查始终是绿的。
