...
- 受支持的 Eve 窗口要求 Workflow Spec v6。每个新 Release 通过生成的 Wrapper 注入 `@evelandhq/workflow-world@0.13.1`——保留 authored agent config，同时强制 `experimental.workflow.world`；固定版本的 World 包在安装时不改动导入源码、`package.json` 与 Lockfile。旧的 `@workflow/world-postgres@5.0.0-beta.34` World 只存在于历史 Release 中，新 Build 永远不会选择它。
