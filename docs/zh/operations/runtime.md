...
共享 World 对新 run 只使用一条完整策略链：显式 `retentionClass` 高于 `workflow-world.retention-class` attribute，attribute 高于 Workflow SDK 的 `$rootRunId`/`$parentRunId` lineage，lineage 高于平台 root invocation context，最后才是 `interactive` 默认值。子 run 直接读取同租户 ancestor 的已存 class，不按 workflow name、timeout 或 callback 猜测；lineage 存在但无法解析时 fail closed。Eve 自身不做 Eveland 专用修改；architecture 门禁从支持的 Eve 发布包读取 `STABLE_WORKFLOW_NAMES`，新增稳定内部 workflow 而未更新审计矩阵时必须失败（最近一次是 Eve 0.48.0 中 `"use workflow"` Tool Body 背后的 Tool Run：从 Turn Step 内部启动、继承祖先的 class，未被回答的 `ask()` 若活过其 Turn 则由 interactive 类截止期回收；再之前是 0.47.3 的 Activity Collector，受 Session Timeout 约束的 root run）。
