...
- **Builds**: at most one running job **per Project**, plus a global cap on concurrently running builds. Worker derives the cap from the machine at startup — `max(1, min(⌊RAM / 4 GiB⌋, cores − 2))`, matching the reference table below — and logs it at boot; `EVELAND_MAX_CONCURRENT_JOBS` overrides it. Builds beyond the cap stay queued while cheap jobs (restart, archive, delete, import) keep flowing, and admission remains one new job per Worker tick (`WORKER_POLL_INTERVAL_MS`, default 5 s). The **Settings → Instance health** Workload section shows current usage as "Running builds N/cap".
