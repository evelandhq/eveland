...
- **Source provenance**: Each Deployment shows where its Release's source came from: the synced commit, or the upload (from the CLI or the Dashboard, by whom, based on which commit, and whether the working tree was dirty). An upload promoted onto a git Project is a hotfix: the page says production has drifted from the repository, and the deployment dialog asks for confirmation before a sync replaces it.
