...
A Project's import kind is immutable and only says how the Project was created. Both kinds accept a multipart upload on `POST /api/projects/:id/sync-source`; the JSON body of the same route still re-clones a git Project from its stored repository URL, which an upload never changes. An upload onto a git Project may deploy (`deploy=true`) and, with `promote=true`, promote as a **hotfix**.

**Source drift.** A git Project whose promoted Release came from an upload has drifted: production runs source the repository does not have. Drift is derived from the production Release's provenance and never stored, so it clears by itself when a synced commit is promoted; zip Projects never drift. `GET /api/projects/:id/deployments` reports it as `sourceDrift` (`{ drifted, production }`), the Deployments page shows a banner, and a JSON `POST /api/projects/:id/sync-source` with `promote: true` is refused with `409` (carrying the same `drift` object) until the body also sends `confirmDrift: true`. A preview sync (`deploy` without `promote`) never needs confirmation.
