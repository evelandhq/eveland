...
The code browser fills the complete project content canvas without an outer frame or page padding. File search is the first control in the tree pane; its borderless toolbar aligns with the equally compact preview header, which shows only the selected path. The tree and header stay fixed while the code body owns both scrolling axes, keeping horizontal scrolling available at the visible bottom edge. Code uses a compact 12 px size with an 18 px line height and muted line numbers. File type, file size, and Eve version metadata stay out of the browser chrome.
