...
After rendering, the bootstrap builds the Dashboard if needed, brings up the infra containers, waits for Postgres, applies migrations, and starts the platform (the admin account is seeded by the API itself at boot from `EVELAND_ADMIN_*`). Once the processes are ready it performs the **implicit CLI login**: the same RFC 8628 device flow `eveland login` runs — same seeded client, same `deploy observe` scopes, a normal revocable token — but approved headlessly over the loopback API with the admin session the bootstrap just created, and stored in the CLI's own credential store (`~/.config/eveland/credentials/`). The golden path `eveland init` → `eveland deploy` therefore never hits a login wall. A failed login is a warning, not a failed start (`eveland login` recovers it). Finally the browser opens on the platform origin (headless installs just print the URL).
