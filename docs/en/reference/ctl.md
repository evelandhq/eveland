...
The bootstrap asks only the decide-per-install questions — public origin (default `http://localhost:17300`), admin email, admin password (blank generates a strong one and prints it once) — and probes the shell for an existing `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` to offer for the built-in agent. Everything else is generated (`APP_SECRET_KEY`, `BETTER_AUTH_SECRET`, gateway/OTLP/scheduler secrets — all CSPRNG, scheduler secrets independent as the worker preflight requires) or derived per OS (Docker-shaped `host.docker.internal` addresses on macOS, loopback on Linux; `EVELAND_RUNTIME` docker vs systemd). The dispatcher activation token is rendered equal to the gateway service token because the API validates activations against exactly that credential. The rendered `etc/eveland.env` is `0600` and written **once** — a resumed bootstrap reuses it verbatim, so secrets are minted exactly once. `--no-prompt` (or no TTY) takes every default; `NODE_ENV=production` means placeholder secrets can never work.

After rendering, the bootstrap builds the Dashboard if needed, brings up the infra containers, waits for Postgres, applies migrations, and starts the platform (the admin account is seeded by the API itself at boot from `EVELAND_ADMIN_*`). Once the processes are ready it performs the **implicit CLI login**: the same RFC 8628 device flow `eveland login` runs — same seeded client, same `deploy observe` scopes, a normal revocable token — but approved headlessly over the loopback API with the admin session the bootstrap just created, and stored in the CLI's own credential store (`~/.config/eveland/credentials/`). The golden path `eveland init` → `eveland deploy` therefore never hits a login wall. A failed login is a warning, not a failed start (`eveland login` recovers it).
