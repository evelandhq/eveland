...
With the token in hand, the bootstrap seeds the **built-in agent** (`stella`): it shells out to the real `eveland` CLI — `eveland deploy templates/starter-agent --name stella` — so seeding walks the exact golden path a user's first deploy walks (preflight, upload, streamed build logs, promote) and cannot silently diverge from it. Model keys collected earlier land in the agent's project environment via `eveland env set` afterwards. Seeding failures warn and print the manual recovery command; the platform is fully usable without the built-in agent. Finally the browser opens on the platform origin (headless installs just print the URL).
...
5. Checkout → refresh the pinned `EVELAND_REVISION` → `pnpm install --frozen-lockfile` → Dashboard build → `db:migrate` → start (with the normal readiness gate).
