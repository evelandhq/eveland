...
| `eveland deploy [dir]` | Validates, packages, uploads, and monitors remote builds; promotion is requested with the upload and completed by the platform inside the build job. Zip projects promote by default (`--no-promote` keeps a preview); git projects deploy previews unless `--promote` makes the upload a hotfix. |
