...
- open Caller Token 用 15–30 分钟的长 TTL(默认 20 分钟),Internal 保持约 60 秒。它不承载
  ...
  作废 Identity Session,不作废已签发的 token。
...
- Agent capture 开关、trace sampling 与 input/output content policy 只作用于
  Eveland 注入的私有 provider，并由运行中的 Agent 动态加载，不重启 Deployment；
  input 与 output content 默认开启，Admin 可以分别关闭；reasoning 属于 output，
  不是独立开关
...
- 拉取或解压源码
- 检查是否为合法 Eve 项目
- 检查 `package.json` 中的 Eve 依赖是否完全限定在平台已验证的 0.29.x、0.30.x 或 0.31.x
...
minor”的三版本滑动窗口；当前窗口是 0.29.x、0.30.x 与 0.31.x。允许精确的 0.29/0.30/0.31
patch、锚定在对应 minor patch 上的 `~`/`^` range，以及 `0.29` / `0.29.x` / `0.29.*`、
`0.30` / `0.30.x` / `0.30.*`、`0.31` / `0.31.x` / `0.31.*`。缺少 Eve 依赖、跨 minor 的宽泛 range 或任何可能解析到
...
UI 仅将当前最新支持线 0.31.x 标为绿色；仍受支持但较旧的 0.29.x/0.30.x 使用红色状态与
...
- `local-dev`：不发送 credential，并且用 loopback Host 调用 Agent。**这只对 Eve
  0.29 的 Agent 构成认证**——它的 `localDev()` 看请求 Host。Eve 0.30 起
