...
self-hosted core services and Agent data plane.

- **API, Agent Gateway, Dashboard, Postgres, and OpenTelemetry Collector** run in Docker Compose. The API and Agent Gateway have no Docker
  socket or host-controller privilege. Agent Gateway also has no `/var/lib/eveland` mount, and the
...
job is accepted.
Agent Gateway listens on host port 4080 and is the only process Traefik forwards wildcard Agent
...
channel paths must reach the same Agent Gateway catch-all as canonical session routes; do not add
path-specific proxy rules that bypass Agent Gateway target selection or cold activation.
...
`/etc/eveland/eveland-worker.env`. Restart API, Agent Gateway, and Dashboard from the
core-services checkout and the Worker from `/opt/eveland`. An instance
intentionally testing `main` uses `EVELAND_RELEASE_CHANNEL=edge` and its exact
revision instead.

The authenticated Dashboard Settings > About page compares Dashboard and API build
identity; API and Agent Gateway also expose it through their existing public
...
configuration for Dashboard, API, Agent Gateway, and Worker. Secrets appear only as a fixed
mask, and connection URLs omit credentials, query values, and fragments. This
diagnostic is intentionally absent from public `/health`: API reads Agent Gateway
...
| `EVELAND_RELEASE_CHANNEL`                          | `dev`                                                                                                          | Product release channel reported by health, logs, and Dashboard About: `dev`, `edge`, `prerelease`, or `stable`. Production tag checkouts use `stable`; `main` test instances use `edge`.                                                                                                                                                                                                                                                                                                               |
| `EVELAND_REVISION`                                 | `unknown`                                                                                                      | Exact deployed Git revision, normally `git rev-parse --short=12 HEAD`. Configure the same value for API, Agent Gateway, Dashboard, and Worker.                                                                                                                                                                                                                                                                                                                                                          |
...
| `EVELAND_OTLP_ENDPOINT`                            | `http://127.0.0.1:4318`                                                                                        | Service-authenticated platform OTLP/HTTP receiver used by API, Agent Gateway, and Worker. Agent receiver endpoints are injected through the runtime policy.                                                                                                                                                                                                                                                                                                                                             |
| `EVELAND_OTLP_SERVICE_TOKEN`                       | _(unset)_                                                                                                      | Required shared secret for API, Agent Gateway, Worker, and Collector platform traffic plus Collector-to-API observability requests. Agents must not receive it.                                                                                                                                                                                                                                                                                                                                         |
...
| `EVELAND_AGENT_BASE_DOMAINS`                       | `agent.localhost`                                                                                              | Comma-separated Host suffix allowlist used by Agent Gateway; the first value is the canonical domain materialized into routes. Production normally uses one value such as `agents.example.com`.                                                                                                                                                                                                                                                                                                         |
...
| `EVELAND_GATEWAY_SERVICE_TOKEN`                    | _(unset)_                                                                                                      | Required shared secret for API/Agent Gateway `/internal/*` calls, including runtime activation; use a long random value and configure it identically on API, worker, and Agent Gateway.                                                                                                                                                                                                                                                                                                                 |
...
| `EVELAND_GATEWAY_MAX_REQUEST_BODY_BYTES`           | `10485760`                                                                                                     | Maximum buffered public request body accepted before Agent Gateway returns 413 without contacting a deployment.                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `EVELAND_API_INTERNAL_URL`                         | `http://127.0.0.1:4000`                                                                                        | Private API origin used by Agent Gateway for service-authenticated dormant Deployment activation. Compose uses `http://api:4000`.                                                                                                                                                                                                                                                                                                                                                                       |
| `EVELAND_ACTIVATION_LEASE_TTL_MS`                  | `180000`                                                                                                       | API lease lifetime for public requests, turns, streams, and ScheduleRuns. Keep it longer than the Agent Gateway renewal interval.                                                                                                                                                                                                                                                                                                                                                                       |
| `EVELAND_PLAYGROUND_SESSION_IDLE_TTL_MS`           | `86400000`                                                                                                     | Playground SessionBinding idle lifetime. Set the same value on API, Agent Gateway, and worker.                                                                                                                                                                                                                                                                                                                                                                                                          |
| `EVELAND_API_SESSION_IDLE_TTL_MS`                  | `604800000`                                                                                                    | Public API SessionBinding idle lifetime. Set the same value on API, Agent Gateway, and worker.                                                                                                                                                                                                                                                                                                                                                                                                          |
| `EVELAND_COLD_START_TIMEOUT_MS`                    | `30000`                                                                                                        | Maximum time Agent Gateway waits for API/Worker to make a dormant Deployment ready before returning 503/504.                                                                                                                                                                                                                                                                                                                                                                                            |
| `EVELAND_ACTIVATION_RENEW_INTERVAL_MS`             | `60000`                                                                                                        | Agent Gateway renewal interval while an upstream response stream is still active.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `EVELAND_SCHEDULER_RUNTIME_SECRET`                 | _(dev fallback only under explicit `NODE_ENV=development`)_                                                    | Required in production on API and worker. Authenticates the injected private Scheduler Channel and its API callback; keep it independent from Agent Gateway and Better Auth secrets.                                                                                                                                                                                                                                                                                                                    |
...
| `WORKFLOW_POSTGRES_URL`                            | _(unset)_                                                                                                      | **Legacy** per-project workflow Postgres base URL. Only needed while pre-cutover legacy Deployments are still being terminated; it lets the worker enumerate and drop the derived `eveland_wf_<project>_<digest>` databases (the role therefore needs `CREATEDB`). New builds never use it, it is no longer required in production, and external-only installs never set it. Reserved from Project Secret overrides.                                                                                    |
...
| `WORKFLOW_POSTGRES_MAX_POOL_SIZE`                  | `10`                                                                                                           | Max pg pool connections each deployment runtime opens against its workflow database; the worker injects it into every deployment and Project Secrets cannot override it. Size the workflow instance's `max_connections` as roughly this value × expected concurrent running deployments, plus the platform pools when both share one Postgres instance. Lower it to fit more deployments per instance; a Postgres `FATAL 53300 "too many clients"` at deployment startup means this budget is exceeded. |
| `EVELAND_WORKFLOW_WORLD_URL`                       | _(unset; required in production)_                                                                              | Deployment-reachable shared database for `@evelandhq/workflow-world`. Every new build bakes the shared world in, so a production worker fails startup closed without this URL.                                                                                                                                                                                                                                                                                                                          |
...
| `EVELAND_PROCESS_MODE`                             | `normal`                                                                                                       | Set `workflow-cutover` on the API and Worker only inside the [external-cutover](./workflow-external-cutover.md) maintenance window: the API serves only the authenticated cutover surface and the Worker claims only jobs stamped with the operation id. Startup fails closed when the mode is set without `EVELAND_WORKFLOW_CUTOVER_OPERATION_ID`; drop the mode again after finalize to return to normal service.                                                                                     |
| `EVELAND_WORKFLOW_CUTOVER_OPERATION_ID`            | _(unset)_                                                                                                      | The cutover operation this process serves. Required by `workflow-cutover` mode on both API and Worker, read by the cutover CLI, and reported on the dispatcher registration so the cutover API can refuse a heartbeat or resume for any other operation.                                                                                                                                                                                                                                                |
| `EVELAND_WORKFLOW_DISPATCHER_START_MODE`           | `normal`                                                                                                       | Dispatcher setting (`eveland-workflow-dispatcher.env`). `recover-paused` — required during the cutover — completes ownership, migrations, and boot recovery but claims nothing until the Control API's authenticated resume (`POST /internal/workflow/dispatcher/resume`) reaches it through the heartbeat reply.                                                                                                                                                                                       |
...
Eve durable Agent Gateway routes also use these existing controls. The affinity secret derives
...
Playground route-auth credentials are not platform session cookies and are not Agent Gateway configuration.
...
path. Agent Gateway accepts that envelope only after `EVELAND_GATEWAY_SERVICE_TOKEN` succeeds, validates its authority
...
owned by Dashboard and completes through the authenticated API; API encrypts one-time ten-minute transactions and
...
retries the original request. Agent Gateway transparently forwards both the challenge and credential;
...
build layer, OTLP signal, API response, Dashboard payload, or worker configuration snapshot.
...
persists `deletion_status = 'deleting'`; Dashboard keeps the Project visible as
`Deleting…`, while mutating platform API requests return `409` until deletion
...
- `EVELAND_WORKFLOW_WORLD_URL` is required when `NODE_ENV=production`; the
...
- Eve 0.38.3 requires workflow spec v6; every new Release injects
  ...
  otherwise match.
- Before any deployment process starts, the worker provisions that project's
...
- Release preparation copies the imported source, moves any authored root
  ...
  `experimental.workflow.world = "@evelandhq/workflow-world"`.
...
Installs with pre-shared-world history run the one-time maintenance-downtime
cutover before their first external-only deploy; fresh installs never need it.
The [workflow external cutover runbook](./workflow-external-cutover.md) is the
executable order of operations, built on `EVELAND_PROCESS_MODE` /
`EVELAND_WORKFLOW_CUTOVER_OPERATION_ID` (API and Worker) and
`EVELAND_WORKFLOW_DISPATCHER_START_MODE=recover-paused` (dispatcher) — see the
...
- Idle: a Deployment may retain its immutable Release, preview route, and
  SessionBindings while no transient unit exists. Cron or Agent Gateway asks API for
...
`EVELAND_ACTIVATION_LEASE_TTL_MS` and `EVELAND_COLD_START_TIMEOUT_MS`, Agent Gateway
...
retains `lastError`, and `stopped` is a normal dormant state. Check API/Agent Gateway
...
1. Budget RAM: `total − 2 GB (OS + core services) − builds × 2 GB` → divide by
   ~0.3 GB for the sustainable running-Agent count.
2. Set `max_connections ≈ agents × WORKFLOW_POSTGRES_MAX_POOL_SIZE + 30
(core services) + WORKFLOW_DISPATCHER_POOL_SIZE + headroom`. Lower the
...
- systemd Deployment processes use `systemd-run --collect` transient units and
  ...
  and the next cron or Agent Gateway request cold-starts the preserved exact Release.
...
continuation token into each Agent Gateway SessionBinding and adds the unique routing index used by public and
Playground reset requests. Apply migrations before rolling API/Agent Gateway/Worker processes. Eve 0.27.6 remote
...
HMAC-keyed Agent Gateway OperationBinding. Apply it before rolling Agent Gateway. OperationBindings use the same
