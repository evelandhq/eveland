...
explicit envelope. `local-dev` is the only method that selects loopback authority; `none`, Basic, Bearer, generic
OIDC, and custom headers use the canonical Project hostname so Eve cannot mistake a public-style request for local development.
