...
describe("route policy", () => {
  test("uses a deterministic 10,000 bucket and honors 90/10 then 50/50 weights", () => {
    ...
    expect(
      keys.filter((key) => selectWeightedTarget(ninetyTen, key)?.deploymentId === "dep_b").length,
    ).toBeGreaterThan(60);
    expect(
      keys.filter((key) => selectWeightedTarget(ninetyTen, key)?.deploymentId === "dep_b").length,
    ).toBeLessThan(140);
    expect(
      keys.filter((key) => selectWeightedTarget(fiftyFifty, key)?.deploymentId === "dep_b").length,
    ).toBeGreaterThan(430);
  });
  ...
  test("scopes deterministic affinity to the route id and policy revision", () => {
    ...
    expect(affinityBucketForRoute("route_a", 7, key)).toBe(
      affinityBucketForRoute("route_a", 7, key),
    );
    expect(affinityBucketForRoute("route_a", 7, key)).not.toBe(
      affinityBucketForRoute("route_a", 8, key),
    );
    expect(affinityBucketForRoute("route_a", 7, key)).not.toBe(
      affinityBucketForRoute("route_b", 7, key),
    );
  });
  ...
  test("rejects more than two targets, duplicate deployments, and invalid totals", () => {
    ...
    expect(() =>
      validateRouteTargets([{ deploymentId: "a", weight: 9_000, variantName: null }]),
    ).toThrow(/10,000/);
    expect(() =>
      validateRouteTargets([
  ...
  test("protects route targets, active bindings, and the three newest artifacts", () => {
    expect(
      isDeploymentProtected({
        deploymentId: "dep_a",
        routeTargetIds: new Set(["dep_a"]),
        activeBindingIds: new Set(),
        retainedIds: new Set(),
      }),
    ).toBe(true);
    expect(
      isDeploymentProtected({
        ...
        activeBindingIds: new Set(["dep_b"]),
        retainedIds: new Set(),
      }),
    ).toBe(true);
    expect(
