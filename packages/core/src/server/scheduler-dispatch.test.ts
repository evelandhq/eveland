...
describe("schedule dispatch credentials", () => {
  test("uses explicit secrets, explicit-development fallbacks, and fails closed otherwise", () => {
    expect(
      resolveSchedulerRuntimeSecret({ EVELAND_SCHEDULER_RUNTIME_SECRET: "explicit-runtime" }),
    ).toBe("explicit-runtime");
    expect(
      resolveSchedulerDispatchSecret({ EVELAND_SCHEDULER_DISPATCH_SECRET: "explicit-dispatch" }),
    ).toBe("explicit-dispatch");
    expect(resolveSchedulerRuntimeSecret({ NODE_ENV: "development" })).toBe(
      "eveland-dev-scheduler-runtime-secret",
    );
    expect(resolveSchedulerDispatchSecret({ NODE_ENV: "development" })).toBe(
      "eveland-dev-scheduler-dispatch-secret",
    );
  ...
  test("round-trips a deployment- and schedule-bound credential", () => {
    ...
    expect(
      verifyScheduleDispatchCredential(credential, secret, new Date("2026-07-15T00:04:59.000Z")),
    ).toEqual(payload);
  });
  ...
  test("rejects tampering, expiration, and short secrets", () => {
    ...
    expect(
      verifyScheduleDispatchCredential(
        `${credential}x`,
        secret,
        new Date("2026-07-15T00:04:59.000Z"),
      ),
    ).toBeNull();
    expect(
      verifyScheduleDispatchCredential(credential, secret, new Date("2026-07-15T00:05:00.000Z")),
    ).toBeNull();
  ...
  test("can authenticate a completion report after the one-time claim window expires", () => {
    ...
    expect(
      verifyScheduleDispatchCredential(credential, secret, new Date("2026-07-15T00:06:00.000Z"), {
        allowExpired: true,
      }),
    ).toEqual(payload);
  });
