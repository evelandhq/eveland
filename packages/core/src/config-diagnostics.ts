...
const definitions: ConfigurationDefinition[] = [
  ...
  entry("EVELAND_DATA_DIR", ["api", "worker"], "Root for managed sources, releases, Agent observability policy, and runtime state.", ".eveland-data"),
  {
  ...
  entry("EVELAND_OTLP_ENDPOINT", ["api", "gateway", "worker"], "Internal OTLP/HTTP endpoint used by Eveland platform telemetry.", "http://127.0.0.1:4318"),
  {
    ...
    components: ["api", "gateway", "worker"],
    sensitivity: "secret",
    purpose: "Authenticates platform producers to the managed Collector and the Collector to Built-in ingest.",
    required: production,
  },
  ...
  entry(
    ...
    ["worker"],
    "Managed Collector container attached to isolated Agent telemetry networks.",
