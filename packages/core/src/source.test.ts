...
describe("inspectEveProject", () => {
  ...
  test("fails closed with an upgrade diagnostic outside the supported Eve line", () => {
    ...
    expect(result.errors).toContain(
      'Unsupported Eve dependency "0.22.6". Eveland requires Eve 0.62.x or 0.63.x. Upgrade the project\'s "eve" dependency before importing or deploying.',
    );
  });

  test("accepts dependency declarations contained inside the 0.62/0.63 compatibility window", () => {
    ...
    // floor": 0.58.x is below the floor now (retired on 2026-09-20 when 0.63
    // entered), 0.55.x retired on 2026-09-19 (when 0.62 entered), 0.54.x retired on 2026-09-16 (when 0.56
    ...
    // 0.61.x were skipped on 2026-09-19 the same way (all below the floor now),
    // and 0.64.x is not admitted before it has passed the matrix.
    for (const version of [
      "0.30.8",
      "0.31.3",
      "0.32.5",
      ...
      // 0.53 on 2026-09-15, 0.54 on 2026-09-16: a formerly supported line is
      ...
      "0.55",
      "0.55.x",
      // Skipped on 2026-09-19, never verified.
      "0.59.1",
      "~0.59.1",
      "^0.59.1",
      "0.59",
      ...
      "0.61",
      "0.61.x",
      // 0.58 was verified and retired on 2026-09-20.
      "0.58.1",
      "~0.58.1",
      "^0.58.1",
      "0.58",
  ...
  test("reports the sliding compatibility window as structured ranges", () => {
    expect(createEveVersionInfo("0.62.0", "src_1")).toEqual({
      version: "0.62.0",
      expected: "0.62.x or 0.63.x",
      supportedRanges: ["0.62.x", "0.63.x"],
      supported: true,
