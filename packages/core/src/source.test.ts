...
describe("inspectEveProject", () => {
  test("recognizes nested eve agent layout and summarizes authored slots", () => {
    const result = inspectEveProject([
      {
        path: "package.json",
        content: JSON.stringify({ name: "weather-agent", dependencies: { eve: "0.54.5" } }),
      },
      {
        path: "agent/agent.ts",
    ...
    expect(result.eveVersion).toBe("0.54.5");
  ...
  test("does not declare eveChat for a non-canonical or unrelated Eve channel", () => {
    const result = inspectEveProject([
      { path: "package.json", content: JSON.stringify({ dependencies: { eve: "0.54.5" } }) },
  ...
  test("fails closed with an upgrade diagnostic outside the supported Eve line", () => {
    ...
    expect(result.errors).toContain(
      'Unsupported Eve dependency "0.22.6". Eveland requires Eve 0.54.x or 0.55.x. Upgrade the project\'s "eve" dependency before importing or deploying.',
    );
  });

  test("accepts dependency declarations contained inside the 0.54/0.55 compatibility window", () => {
    ...
    // floor": 0.52.x and 0.53.x are below the floor now (retired together on
    // 2026-09-15 when 0.55 entered and the window narrowed to two lines),
    // 0.50.x/0.51.x retired before them on 2026-09-12, 0.49.x on 2026-09-07,
    // 0.48.x was never verified (skipped when 0.49.0 superseded it),
    // 0.45.x/0.46.x/0.47.x are further below, and 0.56.x is not admitted
    ...
    for (const version of [
      ...
      "0.49",
      "0.49.x",
      // 0.50 and 0.51 were verified and then retired on 2026-09-12, 0.52 and
      // 0.53 on 2026-09-15: a formerly supported line is rejected exactly like
      // a never-verified one.
      "0.50.0",
      "~0.50.0",
      "^0.50.0",
      "0.50",
  ...
  test("reports the sliding compatibility window as structured ranges", () => {
    expect(createEveVersionInfo("0.54.5", "src_1")).toEqual({
      version: "0.54.5",
      expected: "0.54.x or 0.55.x",
      supportedRanges: ["0.54.x", "0.55.x"],
      supported: true,
