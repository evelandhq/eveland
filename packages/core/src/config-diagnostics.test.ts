...
describe("configuration diagnostics", () => {
  ...
  test("redacts credentials and query values from configured URLs", () => {
    const snapshot = createConfigurationSnapshot("worker", {
      DATABASE_URL:
        "postgres://operator:database-password@db.internal:5432/eveland?sslmode=require&token=query-secret#fragment-secret",
    });
  ...
  test("reports the effective Git clone timeout for the worker", () => {
    ...
    const configured = createConfigurationSnapshot("worker", {
      EVELAND_GIT_CLONE_TIMEOUT_MS: "45000",
    });

    expect(defaults.entries).toContainEqual(
      expect.objectContaining({
        name: "EVELAND_GIT_CLONE_TIMEOUT_MS",
        value: "120000",
        source: "default",
      }),
    );
  ...
  test("redacts the shared database URL", () => {
    const snapshot = createConfigurationSnapshot("worker", {
  ...
  test("reports OpenTelemetry emission and Collector control defaults", () => {
    ...
    expect(snapshot.entries).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          name: "EVELAND_OTEL_COLLECTOR_CONTAINER",
          value: "eveland-otel-collector",
        }),
        expect.objectContaining({
          name: "EVELAND_OTEL_COLLECTOR_IMAGE",
          value: "otel/opentelemetry-collector-contrib:0.149.0",
        }),
      ]),
    );
  });
