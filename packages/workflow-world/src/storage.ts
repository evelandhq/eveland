/**
 * Multi-tenant port of `@workflow/world-postgres`'s storage layer.
 *
 * The event-sourcing logic is upstream's and is kept deliberately close to the
 * original so that upstream fixes remain easy to follow across versions. What
 * changes here is tenancy: every read, write and prepared statement is scoped
 * to the ambient `tenantId`, because all projects now share one database
 * instead of getting one database each.
 *
 * `hooks.getByToken` deserves a note. The design sketch expected it to read the
 * tenant out of the row it found, since a token is its only input. That is not
 * what this does: a world instance always runs inside one deployment and
 * therefore has an ambient tenant, so the lookup is scoped by tenant *and*
 * token. Guessing another tenant's token then resolves to nothing rather than
 * to their hook.
 */
import {
  EntityConflictError,
...
export function createRunsStorage(drizzle: Drizzle, tenantId: string): Storage["runs"] {
  ...
  const get = drizzle
    .select()
    .from(runs)
    .where(and(eq(Schema.runs.tenantId, tenantId), eq(runs.runId, sql.placeholder("id"))))
    .limit(1)
  ...
  return {
    ...
    experimentalSetAttributes: async (
      ...
      const [existing] = await drizzle
        ...
        .from(runs)
        .where(and(eq(Schema.runs.tenantId, tenantId), eq(runs.runId, runId)))
        .limit(1);
      if (!existing) {
  ...
  switch (data.eventType) {
    case "run_cancelled": {
      ...
      await Promise.all([
        drizzle
          .delete(Schema.hooks)
          .where(and(eq(Schema.hooks.tenantId, tenantId), eq(Schema.hooks.runId, runId))),
        drizzle
          .delete(Schema.waits)
          .where(and(eq(Schema.waits.tenantId, tenantId), eq(Schema.waits.runId, runId))),
      ]);
      ...
      return {
        run: updatedRun
          ? (filterRunData(deserializeRunError(compact(updatedRun)), resolveData) as WorkflowRun)
          : undefined,
      };
    }
...
export function createEventsStorage(drizzle: Drizzle, tenantId: string): Storage["events"] {
  ...
  const getStepForValidation = drizzle
    ...
    .where(
      and(
        eq(Schema.steps.tenantId, tenantId),
  ...
  const getWaitForValidation = drizzle
    ...
    .from(Schema.waits)
    .where(
      and(eq(Schema.waits.tenantId, tenantId), eq(Schema.waits.waitId, sql.placeholder("waitId"))),
    )
    .limit(1)
    ...
    async create(runId, data, params): Promise<EventResult> {
          ...
          if (
            ...
            const [inserted] = await drizzle
              ...
              .values({
                ...
                // Upstream casts this to SerializedContent (an array type)
                // even though both the source and the column are keyed
                // objects. The cast is a no-op at runtime and wrong at the
      ...
      if (isHookEventRequiringExistence(data.eventType) && data.correlationId) {
        const [existingHook] = await drizzle
          ...
          .from(Schema.hooks)
          .where(
            and(eq(Schema.hooks.tenantId, tenantId), eq(Schema.hooks.hookId, data.correlationId)),
          )
          .limit(1);
      ...
      if (data.eventType === "run_created") {
        ...
        const [runValue] = await drizzle
          ...
          .values({
            ...
            // See the note on the resilient-start insert above: upstream's cast
      ...
      if (data.eventType === "run_completed") {
        ...
        await Promise.all([
          ...
          drizzle
            .delete(Schema.waits)
            .where(
              and(eq(Schema.waits.tenantId, tenantId), eq(Schema.waits.runId, effectiveRunId)),
            ),
        ]);
      }
      ...
      if (data.eventType === "step_started") {
        value = await drizzle.transaction(async (tx) => {
          ...
          if (!eventValue) {
            throw new EntityConflictError(`Event ${stepStartedEventId} could not be created`);
          }
          return eventValue;
        });
      }
      ...
      if (data.eventType === "hook_created") {
        ...
        if (existingHook) {
          ...
          if (existingHook.runId === effectiveRunId && existingHook.hookId === data.correlationId) {
            ...
            if (!conflictValue) {
              throw new EntityConflictError(`Event ${conflictEventId} could not be created`);
            }
      ...
      if (data.eventType === "wait_completed") {
        ...
        const [waitValue] = await drizzle
          ...
          .where(
            and(
              eq(Schema.waits.tenantId, tenantId),
...
export function createStepsStorage(drizzle: Drizzle, tenantId: string): Storage["steps"] {
  const { steps } = Schema;
    ...
    get: (async (runId, stepId, params) => {
      const [value] = await drizzle
        ...
        .where(
          and(
            eq(Schema.steps.tenantId, tenantId),
            and(eq(steps.runId, runId), eq(steps.stepId, stepId)),
          ),
        )
        .limit(1);
