...
describe("injectSchedulerAdapter", () => {
  test("fails closed outside the 0.45/0.47 compatibility window", async () => {
    for (const eveVersion of [
      ...
      "0.44.4",
      "~0.44.4",
      // 0.46 sits inside the window's hull but is the skipped line -- the
      // union-of-runs peer range is what keeps it out, so it stays here.
      "0.46.0",
      "~0.46.1",
      "0.48.0",
      ">=0.45.2",
      ...
      await expect(injectSchedulerAdapter({ releaseDir })).rejects.toThrow(
        new RegExp(
          `supports Eve 0\\.45\\.x or 0\\.47\\.x.*found ${eveVersion.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
        ),
      );
    }
  });
...
`,
  ...
  test("asks for the queued turn policy on every supported Eve line", async () => {
    ...
    // current 0.45/0.47 window supports the explicit `turnPolicy` option.
    const files = {
      "agent/schedules/zero.ts": `export default { cron: "* * * * *", async run() {} };`,
    };
...
describe("injectExtensionSchedules", () => {
  ...
  test("rejects Extension source paths outside the disposable Release", async () => {
    const releaseDir = await fixture({ eveVersion: "0.45.2", files: {} });
    await injectSchedulerAdapter({ releaseDir });
