...
describe("injectSchedulerAdapter", () => {
  test("fails closed outside the 0.62/0.63 compatibility window", async () => {
    for (const eveVersion of [
      ...
      "^0.61.1",
      "0.61.x",
      // 0.58 retired on 2026-09-20 when 0.63 entered and the window became
      // contiguous again.
      "0.58.1",
      "~0.58.1",
      "^0.58.1",
...
`,
  ...
  test("asks for the queued turn policy on every supported Eve line", async () => {
    ...
    // current 0.62/0.63 window supports the explicit `turnPolicy` option.
    const files = {
      "agent/schedules/zero.ts": `export default { cron: "* * * * *", async run() {} };`,
    };
