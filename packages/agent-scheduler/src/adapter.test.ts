...
describe("injectSchedulerAdapter", () => {
  test("fails closed outside the 0.54/0.55 compatibility window", async () => {
    for (const eveVersion of [
      ...
      // 0.50 and 0.51 on 2026-09-12 when 0.53 and 0.54 entered, and 0.52 and
      // 0.53 on 2026-09-15 when 0.55 entered and the window narrowed to two
      // lines: a formerly supported line is rejected exactly like a
      // never-verified one.
      "0.49.0",
      "~0.49.0",
      "^0.49.0",
...
`,
  ...
  test("asks for the queued turn policy on every supported Eve line", async () => {
    ...
    // current 0.54/0.55 window supports the explicit `turnPolicy` option.
    const files = {
      "agent/schedules/zero.ts": `export default { cron: "* * * * *", async run() {} };`,
    };
...
describe("injectExtensionSchedules", () => {
  ...
  test("rejects Extension source paths outside the disposable Release", async () => {
    const releaseDir = await fixture({ eveVersion: "0.54.5", files: {} });
    await injectSchedulerAdapter({ releaseDir });
