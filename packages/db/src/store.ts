...
export function createMemoryStore(initialState?: Partial<MemoryState>): Store {
  ...
  return {
    ...
    async getAgentConnection(agentConnectionId) {
      return state.agentConnections.find((connection) => connection.id === agentConnectionId) ?? null;
    },

    async getProjectAgentConnection(projectId) {
      return state.agentConnections.find((connection) => connection.target.projectId === projectId) ?? null;
    },
    ...
    async deleteProject(projectId) {
      ...
      const agentConnectionIds = state.agentConnections
        .filter((connection) => connection.target.projectId === projectId)
        .map((connection) => connection.id);
      state.agentAuthCredentials = state.agentAuthCredentials.filter(
        (credential) => !agentConnectionIds.includes(credential.agentConnectionId),
      );
      state.agentAuthTransactions = state.agentAuthTransactions.filter(
        (transaction) => !agentConnectionIds.includes(transaction.agentConnectionId),
      );
...
function agentAuthCredentialMatches(
