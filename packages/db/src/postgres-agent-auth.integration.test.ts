...
describe.skipIf(!database)("Postgres Agent Auth", () => {
  ...
  test("atomically consumes transactions and fences cross-instance refresh leases", async () => {
    const firstStore = createPostgresStore(database!);
    const secondStore = createPostgresStore(database!);
    const project = await firstStore.createProject({ name: `agent-auth-lease-${Date.now()}`, importKind: "zip" });
    const connection = await firstStore.createAgentConnection({
      target: { kind: "managed-project", projectId: project.id },
      method: "oidc",
    ...
    const winner = firstClaim ? { store: firstStore, owner: "instance-a", leaseId: "lease-a" } : { store: secondStore, owner: "instance-b", leaseId: "lease-b" };
    const loser = firstClaim ? { store: secondStore, owner: "instance-b", leaseId: "lease-b" } : { store: firstStore, owner: "instance-a", leaseId: "lease-a" };
    expect([firstClaim, secondClaim].filter(Boolean)).toHaveLength(1);
    await expect(loser.store.completeAgentAuthCredentialRefresh({
      ...key,
      expectedRotationSeq: 0,
      owner: loser.owner,
    ...
    await expect(winner.store.completeAgentAuthCredentialRefresh({
      ...key,
      expectedRotationSeq: 0,
      owner: winner.owner,
    ...
    await expect(winner.store.releaseAgentAuthCredentialRefresh({
      ...key,
      expectedRotationSeq: 0,
      owner: winner.owner,
      leaseId: winner.leaseId,
