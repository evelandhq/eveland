...
// matrix. Eve 0.29/0.30 hand the receive hook a `send` that addresses the
// session with a continuation token; Eve 0.31 hands it `from(address)`. The
// structural cast keeps one source compiling under both type generations.
type ReceiveOps = {
  from?: (address: string) => {
