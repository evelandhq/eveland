...
describe("Eve observer hook compatibility matrix", () => {
  ...
  test.each(compatibilityMatrix)(
    ...
    async ({ packageName }) => {
      ...
      // model-capture's integration is never wrapped — re-verified 2026-09-16
      // (the pinned expression matches 0.55.0 and 0.56.0 alike; 0.55.0 moved
      // the session-init and runtime-global helpers out of the surrounding
      // runtime.js and 0.56.0 added the activation-span metadata block, and
      // both left this arm intact). If the pinned expression changes
