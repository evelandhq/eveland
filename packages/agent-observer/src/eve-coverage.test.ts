...
describe("Eve observer hook compatibility matrix", () => {
  ...
  test.each(compatibilityMatrix)(
    ...
    async ({ packageName }) => {
      ...
      // model-capture's integration is never wrapped — re-verified 2026-09-12
      // (the pinned expression matches 0.52.5, 0.53.1, and 0.54.3 alike; 0.54.0
      // rewrote the surrounding runtime.js for trace schema 4 and left this arm
      // intact). If the pinned expression changes
