...
describe("Eve observer hook compatibility matrix", () => {
  ...
  test.each(compatibilityMatrix)(
    ...
    async ({ packageName }) => {
      ...
      // model-capture's integration is never wrapped — re-verified 2026-09-15
      // (the pinned expression matches 0.54.5 and 0.55.0 alike; 0.54.0 rewrote
      // the surrounding runtime.js for trace schema 4 and 0.55.0 moved the
      // session-init and runtime-global helpers out of it, and both left this
      // arm intact). If the pinned expression changes
