...
describe("Eve observer hook compatibility matrix", () => {
  ...
  test.each(compatibilityMatrix)(
    ...
    async ({ packageName }) => {
      ...
      // model-capture's integration is never wrapped — re-verified 2026-09-10
      // (the pinned expression matches 0.50.0, 0.51.1, and 0.52.5 alike). If the pinned expression changes
