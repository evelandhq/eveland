...
describe("Eve compatibility repository contract", () => {
  test("pins the latest verified Eve patch reviewed for this release", () => {
    expect(LATEST_VERIFIED_EVE_VERSION).toBe("0.63.0");
  });
  ...
  test("keeps the stable Eve workflow retention audit exhaustive", () => {
    const coveredStableWorkflowConstants = [
      ...
      // `TaskExec`: background work is a workflow-tool run there, already
      // audited below. It stays covered only because 0.62.x still exports it;
      // delete the entry when 0.62 retires (the equality check at the end of
      // this test fails on an entry no supported line runs).
      ...
      // and workflow-program tools and so is not in this set). 0.63.0 re-checked
      // 2026-09-20: background invocations moved into this run (its `start.js`
      // still starts it with `startWorkflowOnCurrentDeployment`, now registering
      // a `lifetime` and an origin turn instead of a result kind), and the
      // exported set lost TASK_RUN_WORKFLOW_NAME and nothing else. 0.56.0 rebuilt the `workflow` tool around a
    ...
    // 0.62.x exports six plus the subagent body and 0.63.x the same set minus
  ...
  test("describes the supported 0.62/0.63 compatibility window", () => {
