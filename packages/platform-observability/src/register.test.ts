...
describe("platform observability ESM registration", () => {
  test("instruments Node HTTP before the application module graph loads", async () => {
    const entrypoint = fileURLToPath(new URL("./register-http.test-support.ts", import.meta.url));
    const registerEntrypoint = fileURLToPath(new URL("./register.ts", import.meta.url));
    const { stdout } = await execFileAsync(
      process.execPath,
      ["--import", "tsx", "--import", registerEntrypoint, entrypoint],
      {
