...
async function makeProject(): Promise<string> {
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "probe", dependencies: { eve: "0.54.5" } }),
  );
...
describe("deploy preflight", () => {
  test("packs the tree faithfully: dotfiles, build output, and binaries all ship", async () => {
    ...
    expect(result.eveSpecifier).toBe("0.54.5");
  ...
  test("reads the eve dependency from devDependencies too, like the server", async () => {
    ...
    await writeFile(
      path.join(root, "package.json"),
      JSON.stringify({ name: "dev-probe", devDependencies: { eve: "0.54.5" } }),
    );
  ...
  test("judges eve specifiers against the instance window", () => {
    expect(eveSpecifierProblem("0.54.5", WINDOW)).toBeNull();
    expect(eveSpecifierProblem("^0.54.5", WINDOW)).toBeNull();
    expect(eveSpecifierProblem("0.54.x", WINDOW)).toBeNull();
    expect(eveSpecifierProblem("0.46.0", WINDOW)).toContain("outside this instance's supported");
    // Retired on 2026-09-07 (0.49), 2026-09-12 (0.50, 0.51), and 2026-09-15 (0.52, 0.53): a formerly supported line reads exactly like any other outsider.
    ...
    expect(eveSpecifierProblem("^0.54", WINDOW)).toContain("Unsupported");
    expect(eveSpecifierProblem(">=0.54.0 <0.56.0", WINDOW)).toContain("Unsupported");
