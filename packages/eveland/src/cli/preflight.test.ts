...
describe("deploy preflight", () => {
  ...
  test("judges eve specifiers against the instance window", () => {
    ...
    expect(eveSpecifierProblem("^0.62.0", WINDOW)).toBeNull();
    expect(eveSpecifierProblem("0.64.x", WINDOW)).toBeNull();
    ...
    // 2026-09-23 (0.63, inside today's hull): never verified, so a line
    // between two supported ones reads like any other outsider.
