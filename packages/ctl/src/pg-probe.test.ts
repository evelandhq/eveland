...
describe("libpqEnvironment", () => {
  test("splits a DSN into libpq variables so the password never reaches argv", () => {
    // Everything on a machine can read a process's argv; a backup command
    // must not put a database password there.
    expect(libpqEnvironment("postgres://eveland:s3cr3t@db.internal:6543/eveland_world")).toEqual({
      PGHOST: "db.internal",
      PGPORT: "6543",
      PGDATABASE: "eveland_world",
      PGUSER: "eveland",
      PGPASSWORD: "s3cr3t",
    });
  });

  test("carries sslmode through, and percent-decodes credentials", () => {
    expect(
      libpqEnvironment("postgres://ev%40land:p%3Ass@db.internal/eveland?sslmode=require"),
    ).toEqual({
      PGHOST: "db.internal",
      PGPORT: "5432",
      PGDATABASE: "eveland",
  ...
  test("refuses what it cannot split rather than dumping the wrong database", () => {
    expect(libpqEnvironment("not-a-dsn")).toBeNull();
    // No database name: pg_dump would fall back to the connecting role's name.
    expect(libpqEnvironment("postgres://u:p@db.internal:5432/")).toBeNull();
  });
});
