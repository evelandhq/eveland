...
- `packages/agent-auth`: Node-only Playground authentication registry plus Authorization Code + PKCE OIDC acquisition, encrypted transaction/credential state, verification, refresh, and Basic/Bearer/Vercel-OIDC/custom-header materialization.
...
currently verifies `0.27.13`, `0.28.0`, and `0.29.4`. Eve 0.27.2+ may open an NDJSON stream with a blank byte and lets
...
query, response header, and NDJSON body across Web, API, and Gateway. Eve 0.29.4's registry commands are an
...
- Admins maintain one encrypted, revisioned Shared Agent Environment in System settings. It is injected automatically into every Agent Deployment with precedence Shared Agent Environment < Project Secret < Eveland-reserved values, so a Project can override a shared LLM key. Effective changes restart every running or draining Deployment; API/Web expose only keys, kinds, configured state, and revision, while values stay runtime-only and participate in diagnostic masking. Playground authentication credentials remain separate and reference only Project Secrets.
...
- Playground Agent route auth is configured explicitly through its Playground authentication dialog. `local-dev` alone uses a loopback Host; `none`, Basic, Bearer, Vercel OIDC, custom headers, and generic OIDC use the canonical Project Host. Secret-bearing configuration selects a Project Secret instead of copying the value into the authentication config. Vercel OIDC mirrors Eve 0.29.4 by sending its short-lived token as both Bearer authorization and the trusted deployment header. Eve 0.27 adds standards-compliant route challenges, including Basic realm/UTF-8 metadata, but Eveland still never infers a Playground authentication method from a 401 challenge. Generic OIDC uses discovery, Authorization Code + PKCE, state/nonce, a Web-owned callback, encrypted principal-scoped tokens, explicit JWT/UserInfo verification, refresh-token rotation, singleflight, and Postgres fencing. Register `${WEB_ORIGIN}/agent-auth/oidc/callback` at the IdP. The first pending turn resumes exactly once after callback, a 401 gets at most one refresh/retry, and a 403 never refreshes. Web receives only redacted state, and API resolves the current reference plus security revision for every initial, continuation, cancel, reset, and stream/reconnect request. Eveland never infers a method from Agent source, provider name, or challenge, and the control-plane member id remains only a credential-isolation key rather than the Agent caller. Connection retains Eve's official meaning: an `agent/connections/*` definition that lets an Agent access an external MCP or OpenAPI service.
