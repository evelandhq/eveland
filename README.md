...
Imported Agents must declare an `eve` dependency wholly contained in either verified `0.24.x` or `0.25.x`. Eveland
fails closed during import, build, restart, cold activation, Playground, and public `/eve/v1/session` create, continue, cancel, and stream traffic
...
this is a sliding window: a newly verified minor replaces the older of the two only after changelog/source review
and the complete compatibility matrix pass; an npm publication alone does not widen support.
...
# dormant Eve 0.24.x/0.25.x cron wake, observer usage, idle stop, and continuation wake.
...
- Playground is a fresh, single-conversation AI Elements UI on every page load. Follow-up turns, live reasoning, tool calls/results, HITL responses, and external-authorization prompts stay on one Eve session until the page is reloaded; there is no session switcher. Stopping an Eve 0.25.x or Eve 0.24.5+ turn requests cooperative server cancellation and leaves the event stream open through `turn.cancelled` and the following session boundary. A 404 from an older supported 0.24 patch falls back to aborting only the local stream without marking the platform Session failed. Live raw reasoning and uploaded file bytes are not persisted by the Playground transport.
...
- Admins can create encrypted, revisioned Platform Secret Profiles in System settings and bind one Profile per Project/Deployment consumer. `agent-runtime` uses Project Secret < Project Profile < Deployment Profile < Eveland-reserved precedence and queues deduplicated targeted restarts after effective changes. `agent-connection` is Project-scoped and resolves the current bound value per request. API/Web expose only keys, kinds, binding source, configured state, and revision; Profile values stay runtime-only and are included in diagnostic masking.
...
- Playground Agent route auth is configured explicitly through its Connection dialog. `local-dev` alone uses a loopback Host; `none`, Basic, Bearer, Vercel OIDC, custom headers, and generic OIDC use the canonical Project Host. New secret-bearing configuration selects a Project Secret or an explicitly bound `agent-connection` Platform Secret reference instead of copying the value into Connection config; legacy inline Basic/Bearer/Vercel OIDC config remains readable for compatibility. Vercel OIDC mirrors Eve 0.25.1 by sending its short-lived token as both Bearer authorization and the trusted deployment header. Generic OIDC uses discovery, Authorization Code + PKCE, state/nonce, a Web-owned callback, encrypted principal-scoped tokens, explicit JWT/UserInfo verification, refresh-token rotation, singleflight, and Postgres fencing; confidential client credentials use the same Project/Platform Secret reference catalog. Register `${WEB_ORIGIN}/agent-auth/oidc/callback` at the IdP. The first pending turn resumes exactly once after callback, a 401 gets at most one refresh/retry, and a 403 never refreshes. Web receives only redacted state, and API resolves the current reference plus security revision for every initial, continuation, cancel, and stream/reconnect request. Eveland never infers a method from Agent source, provider name, or challenge, and the control-plane member id remains only a credential-isolation key rather than the Agent caller.
