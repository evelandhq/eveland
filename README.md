...
- Admins maintain one encrypted, revisioned Shared Agent Environment in System settings. It is injected automatically into every Agent Deployment with precedence Shared Agent Environment < Project Secret < Eveland-reserved values, so a Project can override a shared LLM key. Effective changes restart every running or draining Deployment; API/Web expose only keys, kinds, configured state, and revision, while values stay runtime-only and participate in diagnostic masking. Agent Connection credentials remain separate and new configurations reference Project Secrets; existing named Profile bindings/references remain readable during migration but are no longer created by the supported UI/API.
