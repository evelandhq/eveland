...
- `packages/agent-observer`: release-time Eve hook injection for root and directory-form subagents. It bundles an isolated private OpenTelemetry runtime without changing source-owned instrumentation.
...
- `packages/session-collector`: standard OTLP JSON/protobuf decoding and projection into Eveland Session, usage, and instance-health read models.
- `apps/api`: Hono control-plane API with Better Auth email/password sessions and Organization-based team membership/invitations, plus authenticated Built-in OTLP ingest. Its thin app entrypoint composes focused route modules; persistence is supplied by `packages/db`.
...
proxy cannot read imported project sources, Agent observability policies, Collector configuration, or encrypted project secrets:
...
to the workspace's `.eveland-data`; this lets deployment containers mount their
read-only private observability policy and the same durable per-project sandbox
cache across a restart or redeploy.
...
Imported Agents must declare an `eve` dependency wholly contained in verified `0.25.x`, `0.26.x`, or `0.27.x`. Eveland
...
currently verifies `0.25.3`, `0.26.2`, and `0.27.8`. Eve 0.27.2+ may open an NDJSON stream with a blank byte and lets
...
query, response header, and NDJSON body across Web, API, and Gateway. Eve 0.27.8's registry commands are an
...
- API and the host worker share `/var/lib/eveland` at the same absolute path for
  sources, releases, Agent observability policies, and runtime state.
...
- Eveland's injected Eve hook owns private OpenTelemetry providers and exports to the managed Collector without registering or mutating a user's global providers. Source-owned instrumentation and exporters remain unchanged. Admin capture settings control only Eveland's provider and reload without restarting Agent Deployments.
- API, Gateway, and Worker emit their own traces, logs, runtime metrics, and host-capacity metrics through standard OpenTelemetry SDKs to the Collector's service-authenticated platform receiver.
- Token accounting uses Eve's provider-reported `step.completed.data.usage` carried in standard OTLP LogRecords. Input, output, cache-read, cache-write, and optional gateway cost are attributed idempotently to the Eve session node that consumed them. Missing provider usage stays explicitly marked instead of being estimated.
