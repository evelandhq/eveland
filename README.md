...
- Admins maintain one encrypted, revisioned Shared Agent Environment in System settings and explicitly bind it to a Project or one Deployment. Runtime precedence is Project-scoped shared defaults < Deployment-scoped shared defaults < Project Secret < Eveland-reserved values, so a Project can override a shared LLM key. Effective changes queue deduplicated targeted restarts; API/Web expose only keys, kinds, configured state, binding target, and revision, while values stay runtime-only and participate in diagnostic masking. Agent Connection credentials remain separate and new configurations reference Project Secrets; existing named Profile bindings/references remain readable during migration but are no longer created by the UI.
