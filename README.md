...
- `packages/agent-auth`: Node-only generic Agent Connection registry, config normalization/redaction, AES-256-GCM sealing, and Basic/Bearer/custom-header credential materialization.
...
- Playground Agent route auth is configured explicitly through its Connection dialog. `local-dev` alone uses a loopback Host; `none`, Basic, Bearer, and custom headers use the canonical Project Host. Connection secrets are encrypted with `APP_SECRET_KEY`, Web receives only redacted configured state, and API resolves the current security revision again for every initial, continuation, cancel, and stream/reconnect request. Eveland never infers this client method from Agent source or a generic 401, and a control-plane member identity is never sent as the Agent caller.
