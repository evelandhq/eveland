...
currently verifies `0.25.3`, `0.26.2`, and `0.27.6`. Eve 0.27.2+ may open an NDJSON stream with a blank byte and lets
...
`ai@^7.0.34`; the platform workspace uses that peer-compatible range.
...
- Saving, editing, or deleting a Project Variable or Secret queues a targeted restart for every running or draining Deployment so stable, preview, and A/B targets cannot keep stale process environments. New-project setup, Project Settings > Environment, and Shared Agent Environment use the same Type/Name/Value table and modal pattern; both value types are encrypted and never returned after saving. With no live Deployment, the entry is injected on the next deploy.
...
- Schedule runs emit searchable Runtime Logs for pinned-target activation, Scheduler Channel dispatch, and completion, including the ScheduleRun ID and elapsed time. Dispatch timeouts preserve the configured timeout and Deployment context instead of exposing only a generic abort message; credentials and Project Secrets are never included.
