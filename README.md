...
Imported Agents must declare an `eve` dependency wholly contained in verified `0.24.x`, `0.25.x`, or `0.26.x`. Eveland
...
# dormant Eve 0.24.x/0.25.x/0.26.x cron wake, observer usage, idle stop, and continuation wake.
...
- Playground is a fresh, single-conversation AI Elements UI on every page load. Follow-up turns, live reasoning, tool calls/results, HITL responses, and external-authorization prompts stay on one Eve session until the page is reloaded; there is no session switcher. Stopping an Eve 0.25.x, 0.26.x, or Eve 0.24.5+ turn requests cooperative server cancellation and leaves the event stream open through `turn.cancelled` and the following session boundary. Older supported 0.24 patches retain the 404 local-abort fallback. Eve 0.26's Client follows durable streams from the last cursor across transient disconnects. Live raw reasoning and uploaded file bytes are not persisted by the Playground transport.
