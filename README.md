...
- Eveland observability is OTLP end to end. Private providers injected into prepared Eve Releases export Agent traces, logs, and metrics to the managed OpenTelemetry Collector; API, Gateway, and Worker use the platform SDK directly. Built-in is always enabled, accepts service-authenticated standard OTLP/HTTP JSON and protobuf for all three signals, returns standard partial-success rejection counts without dropping valid siblings, and indexes Eveland-owned spans, LogRecords, and Metric Points for its monitoring view while projecting Agent Sessions, usage, and instance health into product read models. Its Platform view aggregates standard platform/runtime spans over a default 24-hour window into per-service request/job/background counts, errors, average latency, and p95 latency, and projects deployment-scoped runtime LogRecords into a recent build/deploy/runtime lifecycle timeline. The managed Collector uses the standard protobuf encoding for Built-in by default. Admins may additionally route every Eveland signal to Elastic, Agent traces to Langfuse, or selected signals/domains to a custom OTLP/HTTP destination. Each exporter has its own retry and persistent queue, while independent empty-OTLP probes expose destination health in System settings.
- The Collector's standard detailed self-metrics are scraped on loopback and routed through OTLP like other Eveland platform metrics. System Observability reports actual exporter send/enqueue failures and queue utilization separately from endpoint probes, while Instance Health treats stale Collector self-metrics as unavailable.
- The Langfuse preset follows its direct-OTLP v4 contract: model calls become generation observations; Agent, Tool, and Subagent operations remain span observations with filterable operation metadata; standard GenAI model/usage attributes and Collector-mapped provider cost stay on the original trace hierarchy.
- Session details query only Eveland-private Agent spans for the Session's root and child Eve identities, rebuild the OpenTelemetry parent/child tree, and attach Agent or platform LogRecords that share its trace/span context. Telemetry emitted by source-owned instrumentation remains in the user's configured backend and is never adopted by this view.
