...
- Every managed Agent has an Agent Connection that selects how Playground obtains Eve Route Auth credentials. Project import does not accept manual Agent Connection configuration. On the first import only, an actual `jinshujuOidc(...)` call in an Eve channel automatically creates or selects the server-managed Jinshuju method; an import alone does not, later syncs do not overwrite a member's setting, and generic 401 responses are never used to guess a method. For every other method, Playground lazily creates `local-dev` when no Connection exists and team members choose `none`, Basic, static Bearer, custom credential headers, or generic OIDC from the Playground Connection settings. API and Worker read Jinshuju OAuth values from `JINSHUJU_OIDC_*` when creating or saving that Connection, then encrypt the standard OIDC config in the same database record used by other methods; changing the environment does not mutate an existing Connection until it is saved again. OIDC uses Authorization Code with PKCE. Generic OIDC only sends `prompt=consent` when `offline_access` is requested; Jinshuju authorization URLs always omit the unsupported `prompt` parameter, including for Connections saved with the former generic consent default. Token endpoint authentication is derived rather than configured: confidential generic clients use `client_secret_basic`, confidential Jinshuju clients use `client_secret_post`, and public clients use `none`. This client-side Agent Connection is unrelated to Eve tool/connection auth.
