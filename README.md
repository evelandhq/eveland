...
- Saving, editing, or deleting a Project Variable or Secret queues a targeted restart for every running or draining Deployment so stable, preview, and A/B targets cannot keep stale process environments. New-project setup, the Project Secrets page, and Shared Agent Environment use the same Type/Name/Value table and modal pattern; both value types are encrypted and never returned after saving. With no live Deployment, the entry is injected on the next deploy.
